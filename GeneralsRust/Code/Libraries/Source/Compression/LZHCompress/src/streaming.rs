...
impl StreamingCompressor {
    ...
    pub fn compress_file<P: AsRef<Path>>(
        ...
        let output_file = File::create(output_path.as_ref()).map_err(|e| LzhError::Io(e))?;
    ...
    pub fn compress_stream<R: Read, W: Write>(
        ...
        writer
            .write_all(&raw_size.to_le_bytes())
            .map_err(|e| LzhError::Io(e))?;
        writer.write_all(&compressed).map_err(|e| LzhError::Io(e))?;
    ...
    pub fn compress_file_mmap<P: AsRef<Path>>(
        ...
        // Write output
        let mut output_file = File::create(output_path.as_ref()).map_err(|e| LzhError::Io(e))?;
        output_file
            .write_all(&(mmap.len() as u32).to_le_bytes())
            .map_err(|e| LzhError::Io(e))?;
        output_file
...
impl StreamingDecompressor {
    ...
    pub fn decompress_file<P: AsRef<Path>>(
        ...
        let mut input_file = File::open(input_path.as_ref()).map_err(|e| LzhError::Io(e))?;
...
mod tests {
    ...
    fn test_decompress_stream() {
        ...
        let mut compressor = StreamingCompressor::new(CompressionLevel::Default);
        let _comp_stats = compressor
            .compress_stream(input, compressed_output)
            .unwrap();
    ...
    fn test_roundtrip_file() {
        ...
        // Compress
        let mut compressor = StreamingCompressor::new(CompressionLevel::Default);
        let comp_stats = compressor
            .compress_file(input_file.path(), compressed_file.path())
            .unwrap();

        println!(
            "Compressed {} bytes to {} bytes ({:.2}% ratio)",
            comp_stats.input_size,
            comp_stats.output_size,
            comp_stats.compression_ratio * 100.0
        );

        // Decompress
        let mut decompressor = StreamingDecompressor::new();
        let decomp_stats = decompressor
    ...
    fn test_with_progress_callback() {
        ...
        let mut compressor =
            StreamingCompressor::new(CompressionLevel::Fast).with_progress(move |input, output| {
    ...
    fn test_custom_buffer_size() {
        let compressor = StreamingCompressor::new(CompressionLevel::Default).with_buffer_size(1024);
