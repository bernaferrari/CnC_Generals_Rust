...
impl BTreeEncoder {
    ...
    fn generate_codes_recursive_impl(
        node: &BTreeNode,
        code: &mut Vec<bool>,
        code_table: &mut [Option<Vec<bool>>; MAX_SYMBOL],
    ) {
        if node.is_leaf() {
    ...
    fn decode_symbol(&self, root: &BTreeNode, bits: &[bool], bit_pos: &mut usize) -> Result<u8> {
        ...
        loop {
            ...
            *bit_pos += 1;
            ...
            .ok_or_else(|| EacError::DecompressionFailed("Invalid tree structure".to_string()))?;
        }
    }
}
