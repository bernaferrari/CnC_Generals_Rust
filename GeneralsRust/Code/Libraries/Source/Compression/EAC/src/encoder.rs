...
impl Encoder {
    ...
    fn encode_single_threaded(&mut self, data: &[u8], compression_type: CompressionType) -> Result<Vec<u8>> {
    ...
    fn encode_parallel(&mut self, data: &[u8], compression_type: CompressionType) -> Result<Vec<u8>> {
    ...
    fn select_optimal_compression(&self, data: &[u8]) -> CompressionType {
        ...
        let (entropy, (repetition_ratio, pattern_count)) = rayon::join(
            ...
            || (self.calculate_repetition_ratio(data), self.calculate_pattern_count(data))
        );
        
        log::debug!(
            "Data analysis: entropy={:.3}, repetition={:.3}, patterns={}",
            entropy, repetition_ratio, pattern_count
        );
...
impl StreamingEncoder {
    ...
    pub fn write(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if self.buffer.len() >= self.encoder.config.chunk_size {
            ...
            let compressed = self.encoder.encode(&self.buffer, CompressionType::RefPack)?;
            self.buffer = chunk;
            Ok(compressed)
        } else {
            Ok(Vec::new())
...
mod tests {
    ...
    fn test_adaptive_compression() {
        ...
        let compressed = encoder.encode(&repetitive_data, CompressionType::RefPack).unwrap();
