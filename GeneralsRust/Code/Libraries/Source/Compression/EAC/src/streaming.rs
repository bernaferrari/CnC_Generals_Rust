...
use crate::{Result, EacError, CompressionType};
...
impl StreamingCompressor {
    ...
    pub fn compress_file<P1, P2>(&mut self, input_path: P1, output_path: P2) -> Result<CompressionStats>
    where
        P1: AsRef<Path>,
        P2: AsRef<Path>,
    {
    ...
    fn compress_with_mmap(&mut self, input_file: File, mut output_file: File, input_size: usize) -> Result<usize> {
    ...
    fn compress_mmap_parallel(&mut self, mmap: &Mmap, output_file: &mut File, chunks_count: usize) -> Result<usize> {
    ...
    fn compress_with_buffers(&mut self, input_file: File, output_file: File, input_size: usize) -> Result<usize> {
...
impl StreamingDecompressor {
    ...
    pub fn decompress_file<P1, P2>(&mut self, input_path: P1, output_path: P2) -> Result<DecompressionStats>
    where
        P1: AsRef<Path>,
        P2: AsRef<Path>,
    {
        ...
        let decompressed_size = if self.config.use_mmap && compressed_size >= self.config.mmap_threshold {
    ...
    fn decompress_with_mmap(&mut self, input_file: File, mut output_file: File, compressed_size: usize) -> Result<usize> {
        log::debug!("Using memory-mapped decompression for {} bytes", compressed_size);
    ...
    fn decompress_with_buffers(&mut self, input_file: File, output_file: File, compressed_size: usize) -> Result<usize> {
...
pub mod async_streaming {
    ...
    impl AsyncStreamingCompressor {
        ...
        pub async fn compress_async<R, W>(&mut self, reader: &mut R, writer: &mut W) -> Result<usize>
        where
            R: AsyncRead + Unpin,
            W: AsyncWrite + Unpin,
        {
            ...
            loop {
                ...
                let chunk = &buffer[..bytes_read];
                let compressed = self.compressor.encoder.encode(chunk, self.compressor.compression_type)?;
        ...
        pub async fn compress_file_async<P1, P2>(&mut self, input_path: P1, output_path: P2) -> Result<CompressionStats>
        where
            P1: AsRef<Path>,
            P2: AsRef<Path>,
        {
            let mut input_file = AsyncFile::open(input_path).await
                .map_err(|e| EacError::Io(e))?;
            let mut output_file = AsyncFile::create(output_path).await
                .map_err(|e| EacError::Io(e))?;
            
            let original_size = input_file.metadata().await
                .map_err(|e| EacError::Io(e))?.len() as usize;
            
            let compressed_size = self.compress_async(&mut input_file, &mut output_file).await?;
    ...
    impl AsyncStreamingDecompressor {
        ...
        pub async fn decompress_async<R, W>(&mut self, reader: &mut R, writer: &mut W) -> Result<usize>
        where
            R: AsyncRead + Unpin,
            W: AsyncWrite + Unpin,
        {
            ...
            loop {
                ...
                // Read compressed chunk
                let mut chunk_buffer = vec![0u8; chunk_size];
                reader.read_exact(&mut chunk_buffer).await
                    .map_err(|e| EacError::Io(e))?;
...
mod tests {
    ...
    fn test_streaming_decompressor() -> Result<()> {
        ...
        let decompress_stats = decompressor.decompress_file(&compressed_path, &decompressed_path)?;

        // Verify
        let decompressed_data = std::fs::read(&decompressed_path).unwrap();
        assert_eq!(test_data, &decompressed_data[..]);
        assert_eq!(compress_stats.original_size, decompress_stats.decompressed_size);
