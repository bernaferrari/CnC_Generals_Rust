...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    for (var i: u32 = 0u; i < light_count; i = i + 1u) {
        ...
        let lambert = max(n_dot_l, 0.2);
        let light_color = light.color.xyz;
        diffuse_sum = diffuse_sum + light_color * lambert * attenuation;
    ...
    let base_contrib = layers.diffuse * diffuse_sum;
    ...
    // Fog affects additive differently - we fade towards black
    if lighting.fog_params.z > 0.0 {
        object_color = vec4<f32>(
            object_color.rgb * in.fog_factor,
            object_color.a
        );
    }

    // Apply FOW visibility (additive blend fades to black)
