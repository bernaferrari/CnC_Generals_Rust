// Decal shader for overlay rendering
// This shader is designed for rendering decals on terrain and objects
// Uses depth testing but no depth writing for proper overlay rendering
...
fn compute_material_layers(
    ...
    for (var stage: u32 = 0u; stage < 8u; stage = stage + 1u) {
        ...
        if hint == 1u {
            ...
            layers.env_stage_mask = layers.env_stage_mask | (1u << stage);
            layers.specular_weight = max(layers.specular_weight, luminance(texel.rgb) * texel.a);
            continue;
        }
        if hint == 3u {
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Alpha testing for decals - discard transparent pixels
