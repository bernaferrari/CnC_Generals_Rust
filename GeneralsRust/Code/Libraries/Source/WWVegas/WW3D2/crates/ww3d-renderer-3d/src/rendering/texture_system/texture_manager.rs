...
#[cfg(test)]
mod examples {
    // Note: These are documentation examples, not runnable tests
    // since they require WGPU context and real files

    #[allow(dead_code)]
    fn example_basic_usage() {
        // This is how you would typically use the texture manager:
        /*
        let device = Arc::new(wgpu_device);
        let queue = Arc::new(wgpu_queue);
        let asset_manager = Arc::new(Mutex::new(AssetManager::new()));

        let mut texture_manager = TextureManager::new(device, queue, asset_manager)?;

        // Load textures for different purposes
        let terrain_texture = texture_manager.load_texture("grass.dds", TextureUsage::Terrain)?;
        let ui_texture = texture_manager.load_texture("button.png", TextureUsage::UI)?;
        let normal_map = texture_manager.load_texture("rock_normal.dds", TextureUsage::Normal)?;

        // Get appropriate samplers
        let terrain_sampler = texture_manager.get_sampler_for_usage(TextureUsage::Terrain);
        let ui_sampler = texture_manager.get_sampler_for_usage(TextureUsage::UI);

        // Bind textures for rendering
        texture_manager.bind_texture(0, terrain_texture);
        texture_manager.bind_texture(1, normal_map);

        // Perform maintenance
        texture_manager.cleanup_cache();

        // Check health
        let health = texture_manager.health_check();
        if !health.overall_healthy {
            println!("Texture manager issues: {:?}", health.issues);
        }
        */
    }

    #[allow(dead_code)]
    fn example_custom_sampler() {
        /*
        let mut texture_manager = /* ... */;

        // Create custom sampler for special effects
        let custom_config = TextureSamplingConfig {
            filter_quality: TextureFilterQuality::Point,
            address_mode_u: TextureAddressMode::MirrorRepeat,
            address_mode_v: TextureAddressMode::ClampToEdge,
            anisotropy_level: 1,
            mipmap_bias: 1.0, // Blurrier
            ..Default::default()
        };

        let custom_sampler = texture_manager.create_custom_sampler(custom_config);
        */
    }
}
