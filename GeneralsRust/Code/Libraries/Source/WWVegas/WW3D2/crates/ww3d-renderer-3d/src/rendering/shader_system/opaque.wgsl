...
/// Compute environment map coordinates from world normal and camera view
/// Used for reflection mapping (types 1, 2, 12, 13, 14, 15)
fn compute_environment_uv(world_normal: vec3<f32>, world_pos: vec3<f32>) -> vec2<f32> {
    // Calculate view vector (from fragment toward camera)
    let view_dir = normalize(camera.eye_position.xyz - world_pos);

    // Calculate reflection vector
    let reflection = view_dir - 2.0 * dot(world_normal, view_dir) * world_normal;

    // Convert reflection to sphere map coordinates
    let m = 2.0 * sqrt(reflection.z + 1.0);
...
/// Used for screen-projected textures (type 3)
fn compute_screen_uv(clip_pos: vec4<f32>) -> vec2<f32> {
    // Perspective divide
...
/// Compute silhouette-based UV
/// Used for rim lighting effects (type 5)
fn compute_silhouette_uv(world_normal: vec3<f32>, world_pos: vec3<f32>) -> vec2<f32> {
    let view_dir = normalize(camera.eye_position.xyz - world_pos);

    // Calculate edge intensity (1.0 at edges, 0.0 facing camera)
    let edge_factor = 1.0 - abs(dot(world_normal, view_dir));

    return vec2<f32>(edge_factor, 0.5);
}
...
fn apply_uv_mapper(uv: vec2<f32>, channel: u32) -> vec2<f32> {
    ...
    // Mapper type 10 = StepLinearOffset
    if mapper_type == 10u {
    ...
    // Mapper type 11 = ZigZagLinearOffset
    if mapper_type == 11u {
        let u_speed = f32(uv_transform.mapper_args[0]) / 1000.0;
        let v_speed = f32(uv_transform.mapper_args[1]) / 1000.0;
    ...
    // Mapper type 18 = BumpEnvironment
    if mapper_type == 18u {
        let u_speed = f32(uv_transform.mapper_args[0]) / 1000.0;
        let v_speed = f32(uv_transform.mapper_args[1]) / 1000.0;
...
fn vs_main(
    ...
    // Pass through texture coordinates with optional UV mapping
    out.tex_coords0 = apply_uv_mapper(vertex.tex_coords0, 0u);
    out.tex_coords1 = apply_uv_mapper(vertex.tex_coords1, 1u);
    out.tex_coords2 = apply_uv_mapper(vertex.tex_coords2, 2u);
    out.tex_coords3 = apply_uv_mapper(vertex.tex_coords3, 3u);
    ...
    out.vertex_diffuse = diffuse_color;
    out.vertex_illumination = illum_color;

    return out;
}

@group(3) @binding(0)
var t_stage0_2d: texture_2d<f32>;
@group(3) @binding(1)
var t_stage0_cube: texture_cube<f32>;
@group(3) @binding(2)
var s_stage0: sampler;

@group(3) @binding(3)
var t_stage1_2d: texture_2d<f32>;
@group(3) @binding(4)
var t_stage1_cube: texture_cube<f32>;
@group(3) @binding(5)
var s_stage1: sampler;

@group(4) @binding(0)
var t_stage2_2d: texture_2d<f32>;
@group(4) @binding(1)
var t_stage2_cube: texture_cube<f32>;
@group(4) @binding(2)
var s_stage2: sampler;

@group(4) @binding(3)
var t_stage3_2d: texture_2d<f32>;
@group(4) @binding(4)
var t_stage3_cube: texture_cube<f32>;
@group(4) @binding(5)
var s_stage3: sampler;
...
@group(7) @binding(0)
var<storage, read> vertex_diffuse_colors: array<vec4<f32>>;
@group(7) @binding(1)
...
/// Apply fog-of-war visibility effects to a color
/// Returns color with visibility alpha applied and explored territory darkening
fn apply_fow_effects(color: vec4<f32>, visibility_alpha: f32, is_explored: f32) -> vec4<f32> {
    var result = color;

    // If explored but not currently visible, darken and desaturate
    if is_explored > 0.5 && visibility_alpha < 0.5 {
        // Desaturate: convert to grayscale
        let gray = dot(result.rgb, vec3<f32>(0.299, 0.587, 0.114));
        result = vec4<f32>(mix(result.rgb, vec3<f32>(gray), 0.7), result.a);

        // Darken: reduce brightness
        result = vec4<f32>(result.rgb * 0.5, result.a);
    }

    // Apply visibility alpha (fade in/out effect)
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let view_dir = safe_normalize(camera.eye_position.xyz - in.world_position);

    let layers = compute_material_layers(in, normal, view_dir);
    ...
    // fixed-function D3DRS_LIGHTING=FALSE output: texture x vertex color,
    // plus texture-cascade emissive/env contributions; no ambient, dynamic
    // light, or specular terms.
    let lighting_enabled = (model.texture_stage_mask.w & (1u << 10u)) != 0u;
    var color_rgb = select(
        layers.diffuse + env_contrib + emissive,
        layers.diffuse * (ambient + diffuse_sum * shadow_factor) + specular * shadow_factor + env_contrib + emissive,
        lighting_enabled
    );
    if blend_mode == 2.0 {
    ...
    if lighting.fog_params.z > 0.0 {
        object_color = vec4<f32>(
            mix(lighting.fog_color.xyz, object_color.rgb, in.fog_factor),
            object_color.a
        );
    }
