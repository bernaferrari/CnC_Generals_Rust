...
/// LCW compression/decompression pipe.
pub struct LcwPipe {
    base: PipeBase,
    control: CompControl,
    counter: usize,
...
impl LcwPipe {
    pub fn new(control: CompControl, block_size: usize) -> Self {
        let safety_margin = block_size / 128 + 1;
        let mut buffer = vec![0u8; block_size + safety_margin];
        let mut buffer2 = vec![0u8; block_size + safety_margin];
                ...
                while slen > 0 {
                    ...
                    if slen > 0 {
                        ...
                        if self.counter == target {
                            ...
                            let mut out =
                                Vec::with_capacity(self.block_header.uncomp_count as usize);
                            if lcw::decompress_to_vec(&compressed, &mut out).is_ok() {
                                total += self.pipe_put(&out[..]);
                            } else {
            ...
            CompControl::Compress => {
                if self.counter > 0 {
                    ...
                    if self.counter == self.block_size {
                        if let Ok(compressed) = lcw::compress(&self.buffer[..self.block_size]) {
                            self.block_header.comp_count = compressed.len() as u16;
                ...
                while slen >= self.block_size {
                    if let Ok(compressed) = lcw::compress(&source[offset..offset + self.block_size])
                    {
                        self.block_header.comp_count = compressed.len() as u16;
                        self.block_header.uncomp_count = self.block_size as u16;
                        total += self.pipe_put(&self.block_header.to_bytes());
    ...
    fn flush(&mut self) -> i32 {
        ...
        if self.counter > 0 {
            match self.control {
                ...
                CompControl::Compress => {
                    if let Ok(compressed) = lcw::compress(&self.buffer[..self.counter]) {
                        self.block_header.comp_count = compressed.len() as u16;
