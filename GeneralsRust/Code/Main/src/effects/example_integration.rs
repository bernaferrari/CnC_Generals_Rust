...
impl GameWithEffects {
    ...
    pub fn render(&self, encoder: &mut wgpu::CommandEncoder, view: &wgpu::TextureView, depth_view: &wgpu::TextureView) {
        ...
        if flash_intensity > 0.01 {
            ...
            info!("Camera flash: color={:?}, intensity={:.2}", flash_color, flash_intensity);
        }
    }
    ...
    pub async fn example_tank_destroys_building(
        ...
        self.effects.create_weapon_fire(
            "tank_cannon",
    ...
    pub async fn example_unit_construction(
        ...
        self.effects.building_constructed(
            factory_pos,
            "WarFactory",
            factory_id,
            &mut self.archive_system,
    ...
    pub async fn example_battle_scene(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Start battle music
        self.effects.play_faction_music(Faction::USA, &mut self.archive_system).await?;
        ...
        for (i, (pos1, pos2)) in battles.iter().enumerate() {
            ...
            self.effects.create_explosion(*pos2, 1.2, "tank", &mut self.archive_system).await?;
    ...
    pub async fn example_environmental_effects(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (effect_type, position) in effects {
            self.effects.create_environmental_effect(
                effect_type,
                position,
                3.0,
                None,
    ...
    pub fn example_quality_adjustment(&mut self) {
        ...
        info!("Performance stats: {:.1} FPS, Particles: {}/{}, Lights: {}/{}, Effects: {}/{}",
              fps,
              stats.particles_used, stats.particles_budget,
              stats.lights_used, stats.lights_budget,
              stats.effects_used, stats.effects_budget);
    ...
    pub fn set_audio_volumes(&mut self, master: f32, music: f32, sfx: f32, voice: f32) {
        ...
        info!("Audio volumes: Master={:.1}%, Music={:.1}%, SFX={:.1}%, Voice={:.1}%",
              master * 100.0, music * 100.0, sfx * 100.0, voice * 100.0);
    }
}
