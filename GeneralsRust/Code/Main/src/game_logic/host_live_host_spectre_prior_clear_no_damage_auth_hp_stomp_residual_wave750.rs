...
pub const RUNTIME_HOST_LIVE_HOST_SPECTRE_PRIOR_CLEAR_NO_DAMAGE_AUTH_HP_STOMP_CMD_NAMES_WAVE750: &[&str] = &[
...
pub enum ResidualHostSpectrePriorClearNoDamageAuthHpStompAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostSpectrePriorClearNoDamageAuthHpStompAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_spectre_prior_clear_no_damage_auth_hp_stomp_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_spectre_prior_clear_no_damage_auth_hp_stomp_last_action() -> ResidualHostSpectrePriorClearNoDamageAuthHpStompAction { ResidualHostSpectrePriorClearNoDamageAuthHpStompAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
fn gl_source() -> &'static str { include_str!("game_logic.rs") }
pub fn honesty_host_spectre_prior_clear_no_damage_auth_hp_stomp_method_names_residual_wave750() -> bool {
    ...
    let ok=residual_name_index(names,"Clear prior gunship residual").is_some()
        && residual_name_index(names,"spectre_gunship_deployment_reg.record_prior_clear").is_some()
        && residual_name_index(names,"gameworld_damage_authority_live").is_some()
        && residual_name_index(names,"Wave 750").is_some()
        && residual_name_index(names,"playable_claim = false").is_some();
    residual_action_store(ResidualHostSpectrePriorClearNoDamageAuthHpStompAction::MethodNames); ok
}
pub fn honesty_host_spectre_prior_clear_no_damage_auth_hp_stomp_source_markers_residual_wave750() -> bool {
    let gl=gl_source();
    ...
    residual_action_store(ResidualHostSpectrePriorClearNoDamageAuthHpStompAction::SourceMarkers); ok
}
pub fn honesty_host_spectre_prior_clear_no_damage_auth_hp_stomp_nav_commands_residual_wave750() -> bool {
    ...
    let ok=residual_name_index(steps,"REQUIRE_DAMAGE_AUTH_SKIP_HP_STOMP").is_some()
        ...
        && residual_name_index(steps,"LIVE_HOST_SPECTRE_PRIOR_CLEAR_NO_DAMAGE_AUTH_HP_STOMP").is_some()
    ...
    residual_action_store(ResidualHostSpectrePriorClearNoDamageAuthHpStompAction::NavCommands); ok
}
pub fn simulate_host_spectre_prior_clear_no_damage_auth_hp_stomp_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostSpectrePriorClearNoDamageAuthHpStompAction::CollectSource); ok
}
pub fn simulate_host_spectre_prior_clear_no_damage_auth_hp_stomp_dispatch_source() -> bool {
    let ok=gl_source().contains("Wave 750")
        && gl_source().contains("record_prior_clear");
    residual_action_store(ResidualHostSpectrePriorClearNoDamageAuthHpStompAction::DispatchSource); ok
}
...
pub fn simulate_live_host_spectre_prior_clear_no_damage_auth_hp_stomp_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostSpectrePriorClearNoDamageAuthHpStompAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_spectre_prior_clear_no_damage_auth_hp_stomp_method_names_residual_wave750()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_spectre_prior_clear_no_damage_auth_hp_stomp_source_markers_residual_wave750()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_spectre_prior_clear_no_damage_auth_hp_stomp_nav_commands_residual_wave750()); }
    #[test] fn sources() { assert!(simulate_host_spectre_prior_clear_no_damage_auth_hp_stomp_collect_source()); assert!(simulate_host_spectre_prior_clear_no_damage_auth_hp_stomp_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_spectre_prior_clear_no_damage_auth_hp_stomp_residual_pack_wave750()); }
    #[test] fn live() { assert!(simulate_live_host_spectre_prior_clear_no_damage_auth_hp_stomp_honesty()); assert!(residual_host_spectre_prior_clear_no_damage_auth_hp_stomp_ok()); }
}
