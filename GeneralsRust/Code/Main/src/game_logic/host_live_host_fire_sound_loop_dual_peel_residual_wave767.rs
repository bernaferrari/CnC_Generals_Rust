...
pub enum ResidualHostFireSoundLoopDualPeelAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostFireSoundLoopDualPeelAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_fire_sound_loop_dual_peel_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_fire_sound_loop_dual_peel_last_action() -> ResidualHostFireSoundLoopDualPeelAction { ResidualHostFireSoundLoopDualPeelAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_fire_sound_loop_dual_peel_method_names_residual_wave767() -> bool {
    ...
    residual_action_store(ResidualHostFireSoundLoopDualPeelAction::MethodNames); ok
}
...
pub fn honesty_host_fire_sound_loop_dual_peel_nav_commands_residual_wave767() -> bool {
    ...
    residual_action_store(ResidualHostFireSoundLoopDualPeelAction::NavCommands); ok
}
pub fn simulate_host_fire_sound_loop_dual_peel_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostFireSoundLoopDualPeelAction::CollectSource); ok
}
pub fn simulate_host_fire_sound_loop_dual_peel_dispatch_source() -> bool {
    let ok=sh_source().contains("host_fire_sound_loop_log::record")
        && sh_source().contains("obj.fire_sound_loop_until_frame = ent.fire_sound_loop_until_frame")
        && !gl_source().contains("obj.tick_fire_sound_loop(self.frame)")
        && include_str!("object.rs").contains("tick_fire_sound_loop(frame)");
    residual_action_store(ResidualHostFireSoundLoopDualPeelAction::DispatchSource); ok
}
...
pub fn simulate_live_host_fire_sound_loop_dual_peel_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostFireSoundLoopDualPeelAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_fire_sound_loop_dual_peel_method_names_residual_wave767()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_fire_sound_loop_dual_peel_source_markers_residual_wave767()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_fire_sound_loop_dual_peel_nav_commands_residual_wave767()); }
    #[test] fn sources() { assert!(simulate_host_fire_sound_loop_dual_peel_collect_source()); assert!(simulate_host_fire_sound_loop_dual_peel_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_fire_sound_loop_dual_peel_residual_pack_wave767()); }
    #[test] fn live() { assert!(simulate_live_host_fire_sound_loop_dual_peel_honesty()); assert!(residual_host_fire_sound_loop_dual_peel_ok()); }
}
