...
pub fn neutron_shell_bezier_point(from: Vec3, to: Vec3, t: f32) -> Vec3 {
    let t = t.clamp(0.0, 1.0);
    let delta = to - from;
    let p0 = from;
    let p3 = to;
    let p1 =
        from + delta * NEUTRON_SHELL_FIRST_PERCENT_INDENT + Vec3::Y * NEUTRON_SHELL_FIRST_HEIGHT;
    let p2 =
        from + delta * NEUTRON_SHELL_SECOND_PERCENT_INDENT + Vec3::Y * NEUTRON_SHELL_SECOND_HEIGHT;
    let u = 1.0 - t;
    let tt = t * t;
    let uu = u * u;
    let uuu = uu * u;
...
/// Apply NukeCannonNeutronWeapon ScatterRadiusVsInfantry residual to aim.
pub fn neutron_shell_scatter_aim(aim: Vec3, target_is_infantry: bool, seed: u32) -> (Vec3, bool) {
    use crate::game_logic::weapon_bootstrap::{host_effective_scatter_radius, scatter_aim_offset};
    let mut scatter = host_effective_scatter_radius(NUKE_CANNON_NEUTRON_WEAPON, target_is_infantry);
    if target_is_infantry && scatter <= 0.0 {
        scatter = NEUTRON_WEAPON_SCATTER_VS_INFANTRY;
    }
    if scatter <= 0.0 {
        return (aim, false);
    }
...
/// Whether NukeCannonNeutronWeapon residual misses intended infantry via ScatterRadiusVsInfantry.
pub fn neutron_shell_scatter_misses_infantry(
    ...
    scatter_misses_intended_target(NEUTRON_WEAPON_SCATTER_VS_INFANTRY, seed, target_hit_radius)
}

/// Wave residual honesty: Neutron shell ScatterRadiusVsInfantry peels (**10**).
pub fn honesty_neutron_shell_scatter_vs_infantry_ok() -> bool {
    use crate::game_logic::weapon_bootstrap::host_effective_scatter_radius;
    let vs = host_effective_scatter_radius(NUKE_CANNON_NEUTRON_WEAPON, true);
    let ground = host_effective_scatter_radius(NUKE_CANNON_NEUTRON_WEAPON, false);
    (NEUTRON_WEAPON_SCATTER_VS_INFANTRY - 10.0).abs() < 0.01
        && (vs - 10.0).abs() < 0.01
        && ground.abs() < 0.01
        && {
            let (sc, applied) = neutron_shell_scatter_aim(Vec3::new(0.0, 0.0, 0.0), true, 3);
            applied && sc.length() <= NEUTRON_WEAPON_SCATTER_VS_INFANTRY + 0.01
        }
}
...
pub fn honesty_neutron_shell_residual_pack_ok() -> bool {
    honesty_neutron_blast_residual_ok()
        ...
        && honesty_neutron_shell_scatter_vs_infantry_ok()
}
...
mod tests {
    ...
    #[test]
    fn neutron_shell_scatter_vs_infantry_peels() {
        assert!(honesty_neutron_shell_scatter_vs_infantry_ok());
        let aim = Vec3::new(200.0, 0.0, 18.0);
        let (no_sc, applied) = neutron_shell_scatter_aim(aim, false, 31);
        assert!(!applied);
        assert_eq!(no_sc, aim);
        let (sc, applied) = neutron_shell_scatter_aim(aim, true, 31);
        assert!(applied);
        let d = ((sc.x - aim.x).powi(2) + (sc.z - aim.z).powi(2)).sqrt();
        assert!(d > 0.01 && d <= NEUTRON_WEAPON_SCATTER_VS_INFANTRY + 0.01);

        assert!(neutron_shell_scatter_misses_infantry(true, 67, 0.5));
        assert!(!neutron_shell_scatter_misses_infantry(true, 67, 100.0));
        assert!(!neutron_shell_scatter_misses_infantry(false, 67, 0.5));
    }
