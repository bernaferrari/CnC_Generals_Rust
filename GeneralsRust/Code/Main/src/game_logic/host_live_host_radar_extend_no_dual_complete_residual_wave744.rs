...
pub enum ResidualHostRadarExtendNoDualCompleteAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostRadarExtendNoDualCompleteAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_radar_extend_no_dual_complete_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_radar_extend_no_dual_complete_last_action() -> ResidualHostRadarExtendNoDualCompleteAction { ResidualHostRadarExtendNoDualCompleteAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
fn gl_source() -> &'static str { include_str!("game_logic.rs") }
fn ready_source() -> &'static str { include_str!("host_radar_extend_ready_log.rs") }
...
pub fn honesty_host_radar_extend_no_dual_complete_method_names_residual_wave744() -> bool {
    ...
    residual_action_store(ResidualHostRadarExtendNoDualCompleteAction::MethodNames); ok
}
pub fn honesty_host_radar_extend_no_dual_complete_source_markers_residual_wave744() -> bool {
    ...
    residual_action_store(ResidualHostRadarExtendNoDualCompleteAction::SourceMarkers); ok
}
pub fn honesty_host_radar_extend_no_dual_complete_nav_commands_residual_wave744() -> bool {
    ...
    residual_action_store(ResidualHostRadarExtendNoDualCompleteAction::NavCommands); ok
}
pub fn simulate_host_radar_extend_no_dual_complete_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostRadarExtendNoDualCompleteAction::CollectSource); ok
}
pub fn simulate_host_radar_extend_no_dual_complete_dispatch_source() -> bool {
    let ok=gl_source().contains("Wave 744")
        && gl_source().contains("shadow_coupled_tick_active()");
    residual_action_store(ResidualHostRadarExtendNoDualCompleteAction::DispatchSource); ok
}
...
pub fn simulate_live_host_radar_extend_no_dual_complete_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostRadarExtendNoDualCompleteAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_radar_extend_no_dual_complete_method_names_residual_wave744()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_radar_extend_no_dual_complete_source_markers_residual_wave744()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_radar_extend_no_dual_complete_nav_commands_residual_wave744()); }
    #[test] fn sources() { assert!(simulate_host_radar_extend_no_dual_complete_collect_source()); assert!(simulate_host_radar_extend_no_dual_complete_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_radar_extend_no_dual_complete_residual_pack_wave744()); }
    #[test] fn live() { assert!(simulate_live_host_radar_extend_no_dual_complete_honesty()); assert!(residual_host_radar_extend_no_dual_complete_ok()); }
}
