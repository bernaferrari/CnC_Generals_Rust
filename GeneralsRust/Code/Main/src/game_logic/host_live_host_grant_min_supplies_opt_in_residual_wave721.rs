...
pub enum ResidualHostGrantMinSuppliesOptInAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostGrantMinSuppliesOptInAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_grant_min_supplies_opt_in_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_grant_min_supplies_opt_in_last_action() -> ResidualHostGrantMinSuppliesOptInAction { ResidualHostGrantMinSuppliesOptInAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_grant_min_supplies_opt_in_method_names_residual_wave721() -> bool {
    ...
    residual_action_store(ResidualHostGrantMinSuppliesOptInAction::MethodNames); ok
}
pub fn honesty_host_grant_min_supplies_opt_in_source_markers_residual_wave721() -> bool {
    ...
    let smoke_ok=smoke.contains("grant_supplies=1")
        && smoke.contains("train_unit|template=AmericaInfantryRanger|force_complete=1|grant_supplies=1")
    ...
    residual_action_store(ResidualHostGrantMinSuppliesOptInAction::SourceMarkers); ok
}
pub fn honesty_host_grant_min_supplies_opt_in_nav_commands_residual_wave721() -> bool {
    ...
    residual_action_store(ResidualHostGrantMinSuppliesOptInAction::NavCommands); ok
}
pub fn simulate_host_grant_min_supplies_opt_in_collect_source() -> bool {
    let ok=eng_source().contains("allow_grant_supplies") && smoke_source().contains("grant_supplies=1");
    residual_action_store(ResidualHostGrantMinSuppliesOptInAction::CollectSource); ok
}
pub fn simulate_host_grant_min_supplies_opt_in_dispatch_source() -> bool {
    let ok=eng_source().contains("Wave 721")
        && smoke_source().contains("train_unit|template=AmericaInfantryRanger|force_complete=1|grant_supplies=1");
    residual_action_store(ResidualHostGrantMinSuppliesOptInAction::DispatchSource); ok
}
...
pub fn simulate_live_host_grant_min_supplies_opt_in_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostGrantMinSuppliesOptInAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_grant_min_supplies_opt_in_method_names_residual_wave721()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_grant_min_supplies_opt_in_source_markers_residual_wave721()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_grant_min_supplies_opt_in_nav_commands_residual_wave721()); }
    #[test] fn sources() { assert!(simulate_host_grant_min_supplies_opt_in_collect_source()); assert!(simulate_host_grant_min_supplies_opt_in_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_grant_min_supplies_opt_in_residual_pack_wave721()); }
    #[test] fn live() { assert!(simulate_live_host_grant_min_supplies_opt_in_honesty()); assert!(residual_host_grant_min_supplies_opt_in_ok()); }
}
