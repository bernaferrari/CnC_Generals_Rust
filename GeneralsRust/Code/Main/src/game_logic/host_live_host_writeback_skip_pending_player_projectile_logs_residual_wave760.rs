...
pub const RUNTIME_HOST_LIVE_HOST_WRITEBACK_SKIP_PENDING_PLAYER_PROJECTILE_LOGS_CMD_NAMES_WAVE760: &[&str] = &[
...
pub enum ResidualHostWritebackSkipPendingPlayerProjectileLogsAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostWritebackSkipPendingPlayerProjectileLogsAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_writeback_skip_pending_player_projectile_logs_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_writeback_skip_pending_player_projectile_logs_last_action() -> ResidualHostWritebackSkipPendingPlayerProjectileLogsAction { ResidualHostWritebackSkipPendingPlayerProjectileLogsAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
fn sh_source() -> &'static str { include_str!("../gameworld_shadow.rs") }
pub fn honesty_host_writeback_skip_pending_player_projectile_logs_method_names_residual_wave760() -> bool {
    ...
    residual_action_store(ResidualHostWritebackSkipPendingPlayerProjectileLogsAction::MethodNames); ok
}
pub fn honesty_host_writeback_skip_pending_player_projectile_logs_source_markers_residual_wave760() -> bool {
    let sh=sh_source();
    ...
    let ok = logs_ok && wbs_ok && sh.contains("Wave 760") && sh.contains("shadow_coupled_tick_active");
    residual_action_store(ResidualHostWritebackSkipPendingPlayerProjectileLogsAction::SourceMarkers);
    ok
}
pub fn honesty_host_writeback_skip_pending_player_projectile_logs_nav_commands_residual_wave760() -> bool {
    ...
    let ok=residual_name_index(steps,"REQUIRE_HAS_PENDING").is_some()
        ...
        && residual_name_index(steps,"LIVE_HOST_WRITEBACK_SKIP_PENDING_PLAYER_PROJECTILE_LOGS").is_some()
        && residual_name_index(steps,"LIVE_PLAYABLE_CLAIM_FALSE").is_some();
    residual_action_store(ResidualHostWritebackSkipPendingPlayerProjectileLogsAction::NavCommands); ok
}
pub fn simulate_host_writeback_skip_pending_player_projectile_logs_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostWritebackSkipPendingPlayerProjectileLogsAction::CollectSource); ok
}
pub fn simulate_host_writeback_skip_pending_player_projectile_logs_dispatch_source() -> bool {
    ...
    residual_action_store(ResidualHostWritebackSkipPendingPlayerProjectileLogsAction::DispatchSource); ok
}
...
pub fn simulate_live_host_writeback_skip_pending_player_projectile_logs_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostWritebackSkipPendingPlayerProjectileLogsAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_writeback_skip_pending_player_projectile_logs_method_names_residual_wave760()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_writeback_skip_pending_player_projectile_logs_source_markers_residual_wave760()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_writeback_skip_pending_player_projectile_logs_nav_commands_residual_wave760()); }
    #[test] fn sources() { assert!(simulate_host_writeback_skip_pending_player_projectile_logs_collect_source()); assert!(simulate_host_writeback_skip_pending_player_projectile_logs_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_writeback_skip_pending_player_projectile_logs_residual_pack_wave760()); }
    #[test] fn live() { assert!(simulate_live_host_writeback_skip_pending_player_projectile_logs_honesty()); assert!(residual_host_writeback_skip_pending_player_projectile_logs_ok()); }
}
