...
pub const RUNTIME_HOST_LIVE_HOST_PRODUCTION_OBJECT_ID_PREFERS_GW_ENTITY_CMD_NAMES_WAVE737: &[&str] = &[
...
pub enum ResidualHostProductionObjectIdPrefersGwEntityAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostProductionObjectIdPrefersGwEntityAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_production_object_id_prefers_gw_entity_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_production_object_id_prefers_gw_entity_last_action() -> ResidualHostProductionObjectIdPrefersGwEntityAction { ResidualHostProductionObjectIdPrefersGwEntityAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_production_object_id_prefers_gw_entity_method_names_residual_wave737() -> bool {
    ...
    residual_action_store(ResidualHostProductionObjectIdPrefersGwEntityAction::MethodNames); ok
}
pub fn honesty_host_production_object_id_prefers_gw_entity_source_markers_residual_wave737() -> bool {
    ...
    residual_action_store(ResidualHostProductionObjectIdPrefersGwEntityAction::SourceMarkers); ok
}
pub fn honesty_host_production_object_id_prefers_gw_entity_nav_commands_residual_wave737() -> bool {
    ...
    residual_action_store(ResidualHostProductionObjectIdPrefersGwEntityAction::NavCommands); ok
}
pub fn simulate_host_production_object_id_prefers_gw_entity_collect_source() -> bool {
    let gl=gl_source();
    let ok=gl.contains("let preferred = ObjectId(raw)")
        && gl.contains("pop_pending_bind");
    residual_action_store(ResidualHostProductionObjectIdPrefersGwEntityAction::CollectSource); ok
}
pub fn simulate_host_production_object_id_prefers_gw_entity_dispatch_source() -> bool {
    let gl=gl_source();
    let ok=gl.contains("Wave 737")
        && gl.contains("saved_next.0.max(after)");
    residual_action_store(ResidualHostProductionObjectIdPrefersGwEntityAction::DispatchSource); ok
}
...
pub fn simulate_live_host_production_object_id_prefers_gw_entity_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostProductionObjectIdPrefersGwEntityAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_production_object_id_prefers_gw_entity_method_names_residual_wave737()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_production_object_id_prefers_gw_entity_source_markers_residual_wave737()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_production_object_id_prefers_gw_entity_nav_commands_residual_wave737()); }
    #[test] fn sources() { assert!(simulate_host_production_object_id_prefers_gw_entity_collect_source()); assert!(simulate_host_production_object_id_prefers_gw_entity_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_production_object_id_prefers_gw_entity_residual_pack_wave737()); }
    #[test] fn live() { assert!(simulate_live_host_production_object_id_prefers_gw_entity_honesty()); assert!(residual_host_production_object_id_prefers_gw_entity_ok()); }
}
