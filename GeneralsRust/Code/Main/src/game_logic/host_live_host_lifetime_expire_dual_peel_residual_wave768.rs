...
pub enum ResidualHostLifetimeExpireDualPeelAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostLifetimeExpireDualPeelAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_lifetime_expire_dual_peel_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_lifetime_expire_dual_peel_last_action() -> ResidualHostLifetimeExpireDualPeelAction { ResidualHostLifetimeExpireDualPeelAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_lifetime_expire_dual_peel_method_names_residual_wave768() -> bool {
    ...
    residual_action_store(ResidualHostLifetimeExpireDualPeelAction::MethodNames); ok
}
...
pub fn honesty_host_lifetime_expire_dual_peel_nav_commands_residual_wave768() -> bool {
    ...
    residual_action_store(ResidualHostLifetimeExpireDualPeelAction::NavCommands); ok
}
pub fn simulate_host_lifetime_expire_dual_peel_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostLifetimeExpireDualPeelAction::CollectSource); ok
}
pub fn simulate_host_lifetime_expire_dual_peel_dispatch_source() -> bool {
    ...
    residual_action_store(ResidualHostLifetimeExpireDualPeelAction::DispatchSource); ok
}
...
pub fn simulate_live_host_lifetime_expire_dual_peel_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostLifetimeExpireDualPeelAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_lifetime_expire_dual_peel_method_names_residual_wave768()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_lifetime_expire_dual_peel_source_markers_residual_wave768()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_lifetime_expire_dual_peel_nav_commands_residual_wave768()); }
    #[test] fn sources() { assert!(simulate_host_lifetime_expire_dual_peel_collect_source()); assert!(simulate_host_lifetime_expire_dual_peel_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_lifetime_expire_dual_peel_residual_pack_wave768()); }
    #[test] fn live() { assert!(simulate_live_host_lifetime_expire_dual_peel_honesty()); assert!(residual_host_lifetime_expire_dual_peel_ok()); }
}
