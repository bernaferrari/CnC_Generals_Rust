...
pub enum ResidualHostProductionReadyPoseAuthorityAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostProductionReadyPoseAuthorityAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_production_ready_pose_authority_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_production_ready_pose_authority_last_action() -> ResidualHostProductionReadyPoseAuthorityAction { ResidualHostProductionReadyPoseAuthorityAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
fn gl_source() -> &'static str { include_str!("game_logic.rs") }
fn ready_source() -> &'static str { include_str!("host_production_ready_log.rs") }
...
pub fn honesty_host_production_ready_pose_authority_method_names_residual_wave735() -> bool {
    ...
    residual_action_store(ResidualHostProductionReadyPoseAuthorityAction::MethodNames); ok
}
pub fn honesty_host_production_ready_pose_authority_source_markers_residual_wave735() -> bool {
    ...
    residual_action_store(ResidualHostProductionReadyPoseAuthorityAction::SourceMarkers); ok
}
pub fn honesty_host_production_ready_pose_authority_nav_commands_residual_wave735() -> bool {
    ...
    residual_action_store(ResidualHostProductionReadyPoseAuthorityAction::NavCommands); ok
}
pub fn simulate_host_production_ready_pose_authority_collect_source() -> bool {
    let ok=ready_source().contains("record_with_pose")
        && gl_source().contains("ready_by_producer");
    residual_action_store(ResidualHostProductionReadyPoseAuthorityAction::CollectSource); ok
}
pub fn simulate_host_production_ready_pose_authority_dispatch_source() -> bool {
    ...
    residual_action_store(ResidualHostProductionReadyPoseAuthorityAction::DispatchSource); ok
}
...
pub fn simulate_live_host_production_ready_pose_authority_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostProductionReadyPoseAuthorityAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_production_ready_pose_authority_method_names_residual_wave735()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_production_ready_pose_authority_source_markers_residual_wave735()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_production_ready_pose_authority_nav_commands_residual_wave735()); }
    #[test] fn sources() { assert!(simulate_host_production_ready_pose_authority_collect_source()); assert!(simulate_host_production_ready_pose_authority_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_production_ready_pose_authority_residual_pack_wave735()); }
    #[test] fn live() { assert!(simulate_live_host_production_ready_pose_authority_honesty()); assert!(residual_host_production_ready_pose_authority_ok()); }
}
