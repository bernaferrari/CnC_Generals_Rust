...
pub const RUNTIME_HOST_LIVE_HOST_CRUSH_FAILCLOSED_NO_DAMAGE_AUTH_HP_STOMP_CMD_NAMES_WAVE746: &[&str] = &[
...
pub enum ResidualHostCrushFailclosedNoDamageAuthHpStompAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostCrushFailclosedNoDamageAuthHpStompAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_crush_failclosed_no_damage_auth_hp_stomp_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_crush_failclosed_no_damage_auth_hp_stomp_last_action() -> ResidualHostCrushFailclosedNoDamageAuthHpStompAction { ResidualHostCrushFailclosedNoDamageAuthHpStompAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_crush_failclosed_no_damage_auth_hp_stomp_method_names_residual_wave746() -> bool {
    ...
    residual_action_store(ResidualHostCrushFailclosedNoDamageAuthHpStompAction::MethodNames); ok
}
pub fn honesty_host_crush_failclosed_no_damage_auth_hp_stomp_source_markers_residual_wave746() -> bool {
    let gl=gl_source();
    ...
    residual_action_store(ResidualHostCrushFailclosedNoDamageAuthHpStompAction::SourceMarkers); ok
}
pub fn honesty_host_crush_failclosed_no_damage_auth_hp_stomp_nav_commands_residual_wave746() -> bool {
    ...
    let ok=residual_name_index(steps,"REQUIRE_DAMAGE_AUTH_SKIP_HP_STOMP").is_some()
        ...
        && residual_name_index(steps,"LIVE_HOST_CRUSH_FAILCLOSED_NO_DAMAGE_AUTH_HP_STOMP").is_some()
    ...
    residual_action_store(ResidualHostCrushFailclosedNoDamageAuthHpStompAction::NavCommands); ok
}
pub fn simulate_host_crush_failclosed_no_damage_auth_hp_stomp_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostCrushFailclosedNoDamageAuthHpStompAction::CollectSource); ok
}
pub fn simulate_host_crush_failclosed_no_damage_auth_hp_stomp_dispatch_source() -> bool {
    let ok=gl_source().contains("Wave 746")
        && gl_source().contains("host_damage_log::record");
    residual_action_store(ResidualHostCrushFailclosedNoDamageAuthHpStompAction::DispatchSource); ok
}
...
pub fn simulate_live_host_crush_failclosed_no_damage_auth_hp_stomp_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostCrushFailclosedNoDamageAuthHpStompAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_crush_failclosed_no_damage_auth_hp_stomp_method_names_residual_wave746()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_crush_failclosed_no_damage_auth_hp_stomp_source_markers_residual_wave746()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_crush_failclosed_no_damage_auth_hp_stomp_nav_commands_residual_wave746()); }
    #[test] fn sources() { assert!(simulate_host_crush_failclosed_no_damage_auth_hp_stomp_collect_source()); assert!(simulate_host_crush_failclosed_no_damage_auth_hp_stomp_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_crush_failclosed_no_damage_auth_hp_stomp_residual_pack_wave746()); }
    #[test] fn live() { assert!(simulate_live_host_crush_failclosed_no_damage_auth_hp_stomp_honesty()); assert!(residual_host_crush_failclosed_no_damage_auth_hp_stomp_ok()); }
}
