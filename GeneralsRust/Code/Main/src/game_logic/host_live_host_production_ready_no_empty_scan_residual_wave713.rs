...
pub enum ResidualHostProductionReadyNoEmptyScanAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostProductionReadyNoEmptyScanAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_production_ready_no_empty_scan_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_production_ready_no_empty_scan_last_action() -> ResidualHostProductionReadyNoEmptyScanAction { ResidualHostProductionReadyNoEmptyScanAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_production_ready_no_empty_scan_method_names_residual_wave713() -> bool {
    ...
    residual_action_store(ResidualHostProductionReadyNoEmptyScanAction::MethodNames); ok
}
pub fn honesty_host_production_ready_no_empty_scan_source_markers_residual_wave713() -> bool {
    ...
    residual_action_store(ResidualHostProductionReadyNoEmptyScanAction::SourceMarkers); ok
}
pub fn honesty_host_production_ready_no_empty_scan_nav_commands_residual_wave713() -> bool {
    ...
    residual_action_store(ResidualHostProductionReadyNoEmptyScanAction::NavCommands); ok
}
pub fn simulate_host_production_ready_no_empty_scan_collect_source() -> bool {
    let gl=gl_source();
    let ok=gl.contains("ready_producers.contains(&id)") && gl.contains("host_production_ready_log::drain");
    residual_action_store(ResidualHostProductionReadyNoEmptyScanAction::CollectSource); ok
}
pub fn simulate_host_production_ready_no_empty_scan_dispatch_source() -> bool {
    let ok=gl_source().contains("Wave 713: empty ready log") || gl_source().contains("Wave 617/713");
    residual_action_store(ResidualHostProductionReadyNoEmptyScanAction::DispatchSource); ok
}
...
pub fn simulate_live_host_production_ready_no_empty_scan_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostProductionReadyNoEmptyScanAction::Composite); }
    ok
}
...
mod tests {
    ...
    #[test] fn method_names_residual() { assert!(honesty_host_production_ready_no_empty_scan_method_names_residual_wave713()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_production_ready_no_empty_scan_source_markers_residual_wave713()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_production_ready_no_empty_scan_nav_commands_residual_wave713()); }
    #[test] fn sources() { assert!(simulate_host_production_ready_no_empty_scan_collect_source()); assert!(simulate_host_production_ready_no_empty_scan_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_production_ready_no_empty_scan_residual_pack_wave713()); }
    #[test] fn live() { assert!(simulate_live_host_production_ready_no_empty_scan_honesty()); assert!(residual_host_production_ready_no_empty_scan_ok()); }
    ...
    fn sole_tick_empty_ready_log_does_not_complete_queue_head() {
        ...
        assert!(upg.is_empty(), "upgrade completions without ready log: {upg:?}");
        assert!(units.is_empty(), "unit completions without ready log: {units:?}");
        let qlen = logic
            .get_objects()
            .get(&id)
        ...
        assert_eq!(units2.len(), 1, "ready membership must complete unit: {units2:?}");
