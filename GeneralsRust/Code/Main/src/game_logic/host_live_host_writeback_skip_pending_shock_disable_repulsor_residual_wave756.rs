...
pub const RUNTIME_HOST_LIVE_HOST_WRITEBACK_SKIP_PENDING_SHOCK_DISABLE_REPULSOR_CMD_NAMES_WAVE756: &[&str] = &[
...
pub enum ResidualHostWritebackSkipPendingShockDisableRepulsorAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostWritebackSkipPendingShockDisableRepulsorAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_writeback_skip_pending_shock_disable_repulsor_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_writeback_skip_pending_shock_disable_repulsor_last_action() -> ResidualHostWritebackSkipPendingShockDisableRepulsorAction { ResidualHostWritebackSkipPendingShockDisableRepulsorAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
fn sh_source() -> &'static str { include_str!("../gameworld_shadow.rs") }
fn shock_log() -> &'static str { include_str!("host_shock_stun_log.rs") }
fn disable_log() -> &'static str { include_str!("host_disable_timers_log.rs") }
fn repulsor_log() -> &'static str { include_str!("host_repulsor_log.rs") }
pub fn honesty_host_writeback_skip_pending_shock_disable_repulsor_method_names_residual_wave756() -> bool {
    ...
    residual_action_store(ResidualHostWritebackSkipPendingShockDisableRepulsorAction::MethodNames); ok
}
pub fn honesty_host_writeback_skip_pending_shock_disable_repulsor_source_markers_residual_wave756() -> bool {
    let sh=sh_source();
    ...
    residual_action_store(ResidualHostWritebackSkipPendingShockDisableRepulsorAction::SourceMarkers); ok
}
pub fn honesty_host_writeback_skip_pending_shock_disable_repulsor_nav_commands_residual_wave756() -> bool {
    let steps=LIVE_HOST_WRITEBACK_SKIP_PENDING_SHOCK_DISABLE_REPULSOR_NAV_STEPS_WAVE756;
    let cmds=RUNTIME_HOST_LIVE_HOST_WRITEBACK_SKIP_PENDING_SHOCK_DISABLE_REPULSOR_CMD_NAMES_WAVE756;
    let ok=residual_name_index(steps,"REQUIRE_HAS_PENDING").is_some()
        ...
        && residual_name_index(steps,"LIVE_HOST_WRITEBACK_SKIP_PENDING_SHOCK_DISABLE_REPULSOR").is_some()
        && residual_name_index(steps,"LIVE_PLAYABLE_CLAIM_FALSE").is_some()
        && residual_name_index(cmds,"host_writeback_skip_pending_shock_disable_repulsor").is_some()
    ...
    residual_action_store(ResidualHostWritebackSkipPendingShockDisableRepulsorAction::NavCommands); ok
}
pub fn simulate_host_writeback_skip_pending_shock_disable_repulsor_collect_source() -> bool {
    let ok=sh_source().contains("Wave 756")
        && shock_log().contains("has_pending");
    residual_action_store(ResidualHostWritebackSkipPendingShockDisableRepulsorAction::CollectSource); ok
}
pub fn simulate_host_writeback_skip_pending_shock_disable_repulsor_dispatch_source() -> bool {
    ...
    residual_action_store(ResidualHostWritebackSkipPendingShockDisableRepulsorAction::DispatchSource); ok
}
...
pub fn simulate_live_host_writeback_skip_pending_shock_disable_repulsor_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostWritebackSkipPendingShockDisableRepulsorAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_writeback_skip_pending_shock_disable_repulsor_method_names_residual_wave756()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_writeback_skip_pending_shock_disable_repulsor_source_markers_residual_wave756()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_writeback_skip_pending_shock_disable_repulsor_nav_commands_residual_wave756()); }
    #[test] fn sources() { assert!(simulate_host_writeback_skip_pending_shock_disable_repulsor_collect_source()); assert!(simulate_host_writeback_skip_pending_shock_disable_repulsor_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_writeback_skip_pending_shock_disable_repulsor_residual_pack_wave756()); }
    #[test] fn live() { assert!(simulate_live_host_writeback_skip_pending_shock_disable_repulsor_honesty()); assert!(residual_host_writeback_skip_pending_shock_disable_repulsor_ok()); }
}
