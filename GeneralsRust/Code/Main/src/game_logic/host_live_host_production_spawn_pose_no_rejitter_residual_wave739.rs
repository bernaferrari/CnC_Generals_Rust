...
pub enum ResidualHostProductionSpawnPoseNoRejitterAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostProductionSpawnPoseNoRejitterAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_production_spawn_pose_no_rejitter_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_production_spawn_pose_no_rejitter_last_action() -> ResidualHostProductionSpawnPoseNoRejitterAction { ResidualHostProductionSpawnPoseNoRejitterAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_production_spawn_pose_no_rejitter_method_names_residual_wave739() -> bool {
    ...
    residual_action_store(ResidualHostProductionSpawnPoseNoRejitterAction::MethodNames); ok
}
pub fn honesty_host_production_spawn_pose_no_rejitter_source_markers_residual_wave739() -> bool {
    let gl=gl_source();
    let j=gl.find("fn host_apply_production_spawn_ready_completions").unwrap_or(0);
    let body=&gl[j..j+4500.min(gl.len().saturating_sub(j))];
    let ok=body.contains("Wave 739")
        && body.contains("let sole = crate::gameworld_shadow::gameworld_production_sole_tick_enabled()")
        && body.contains("if sole {")
        && body.contains("Vec3::ZERO")
        && body.contains("if !sole {")
        && body.contains("selection_radius")
        && !gl.contains("playable_claim = true");
    residual_action_store(ResidualHostProductionSpawnPoseNoRejitterAction::SourceMarkers); ok
}
pub fn honesty_host_production_spawn_pose_no_rejitter_nav_commands_residual_wave739() -> bool {
    ...
    residual_action_store(ResidualHostProductionSpawnPoseNoRejitterAction::NavCommands); ok
}
pub fn simulate_host_production_spawn_pose_no_rejitter_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostProductionSpawnPoseNoRejitterAction::CollectSource); ok
}
pub fn simulate_host_production_spawn_pose_no_rejitter_dispatch_source() -> bool {
    let gl=gl_source();
    let ok=gl.contains("Wave 739")
        && gl.contains("if !sole {");
    residual_action_store(ResidualHostProductionSpawnPoseNoRejitterAction::DispatchSource); ok
}
...
pub fn simulate_live_host_production_spawn_pose_no_rejitter_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostProductionSpawnPoseNoRejitterAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_production_spawn_pose_no_rejitter_method_names_residual_wave739()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_production_spawn_pose_no_rejitter_source_markers_residual_wave739()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_production_spawn_pose_no_rejitter_nav_commands_residual_wave739()); }
    #[test] fn sources() { assert!(simulate_host_production_spawn_pose_no_rejitter_collect_source()); assert!(simulate_host_production_spawn_pose_no_rejitter_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_production_spawn_pose_no_rejitter_residual_pack_wave739()); }
    #[test] fn live() { assert!(simulate_live_host_production_spawn_pose_no_rejitter_honesty()); assert!(residual_host_production_spawn_pose_no_rejitter_ok()); }
}
