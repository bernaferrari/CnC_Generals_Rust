...
pub enum ResidualHostAliasFallbackOptInAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostAliasFallbackOptInAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_alias_fallback_opt_in_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_alias_fallback_opt_in_last_action() -> ResidualHostAliasFallbackOptInAction { ResidualHostAliasFallbackOptInAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_alias_fallback_opt_in_method_names_residual_wave725() -> bool {
    ...
    residual_action_store(ResidualHostAliasFallbackOptInAction::MethodNames); ok
}
pub fn honesty_host_alias_fallback_opt_in_source_markers_residual_wave725() -> bool {
    ...
    let ok=eng_ok&&train_exact&&construct_exact&&smoke_ok&&!eng.contains("playable_claim = true");
    residual_action_store(ResidualHostAliasFallbackOptInAction::SourceMarkers); ok
}
pub fn honesty_host_alias_fallback_opt_in_nav_commands_residual_wave725() -> bool {
    ...
    residual_action_store(ResidualHostAliasFallbackOptInAction::NavCommands); ok
}
pub fn simulate_host_alias_fallback_opt_in_collect_source() -> bool {
    let ok=eng_source().contains("allow_alias_fallback") && smoke_source().contains("alias_fallback=1");
    residual_action_store(ResidualHostAliasFallbackOptInAction::CollectSource); ok
}
pub fn simulate_host_alias_fallback_opt_in_dispatch_source() -> bool {
    ...
    residual_action_store(ResidualHostAliasFallbackOptInAction::DispatchSource); ok
}
...
pub fn simulate_live_host_alias_fallback_opt_in_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostAliasFallbackOptInAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_alias_fallback_opt_in_method_names_residual_wave725()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_alias_fallback_opt_in_source_markers_residual_wave725()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_alias_fallback_opt_in_nav_commands_residual_wave725()); }
    #[test] fn sources() { assert!(simulate_host_alias_fallback_opt_in_collect_source()); assert!(simulate_host_alias_fallback_opt_in_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_alias_fallback_opt_in_residual_pack_wave725()); }
    #[test] fn live() { assert!(simulate_live_host_alias_fallback_opt_in_honesty()); assert!(residual_host_alias_fallback_opt_in_ok()); }
}
