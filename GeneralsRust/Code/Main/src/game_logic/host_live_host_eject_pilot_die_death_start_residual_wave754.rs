...
pub enum ResidualHostEjectPilotDieDeathStartAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostEjectPilotDieDeathStartAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_eject_pilot_die_death_start_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_eject_pilot_die_death_start_last_action() -> ResidualHostEjectPilotDieDeathStartAction { ResidualHostEjectPilotDieDeathStartAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
fn gl_source() -> &'static str { include_str!("game_logic.rs") }
fn obj_source() -> &'static str { include_str!("object.rs") }
pub fn honesty_host_eject_pilot_die_death_start_method_names_residual_wave754() -> bool {
    ...
    residual_action_store(ResidualHostEjectPilotDieDeathStartAction::MethodNames); ok
}
pub fn honesty_host_eject_pilot_die_death_start_source_markers_residual_wave754() -> bool {
    ...
    residual_action_store(ResidualHostEjectPilotDieDeathStartAction::SourceMarkers); ok
}
pub fn honesty_host_eject_pilot_die_death_start_nav_commands_residual_wave754() -> bool {
    ...
    residual_action_store(ResidualHostEjectPilotDieDeathStartAction::NavCommands); ok
}
pub fn simulate_host_eject_pilot_die_death_start_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostEjectPilotDieDeathStartAction::CollectSource); ok
}
pub fn simulate_host_eject_pilot_die_death_start_dispatch_source() -> bool {
    ...
    residual_action_store(ResidualHostEjectPilotDieDeathStartAction::DispatchSource); ok
}
...
pub fn simulate_live_host_eject_pilot_die_death_start_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostEjectPilotDieDeathStartAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_eject_pilot_die_death_start_method_names_residual_wave754()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_eject_pilot_die_death_start_source_markers_residual_wave754()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_eject_pilot_die_death_start_nav_commands_residual_wave754()); }
    #[test] fn sources() { assert!(simulate_host_eject_pilot_die_death_start_collect_source()); assert!(simulate_host_eject_pilot_die_death_start_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_eject_pilot_die_death_start_residual_pack_wave754()); }
    #[test] fn live() { assert!(simulate_live_host_eject_pilot_die_death_start_honesty()); assert!(residual_host_eject_pilot_die_death_start_ok()); }
}
