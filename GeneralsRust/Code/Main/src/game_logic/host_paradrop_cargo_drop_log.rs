...
pub fn record_drop(ev: ParadropCargoDropEvent) { DROPS.with(|l| l.borrow_mut().push(ev)); }
pub fn record_ground(ev: ParadropParachuteGroundEvent) { GROUND.with(|l| l.borrow_mut().push(ev)); }
pub fn drain_drops() -> Vec<ParadropCargoDropEvent> { DROPS.with(|l| std::mem::take(&mut *l.borrow_mut())) }
pub fn drain_ground() -> Vec<ParadropParachuteGroundEvent> { GROUND.with(|l| std::mem::take(&mut *l.borrow_mut())) }
pub fn clear() {
