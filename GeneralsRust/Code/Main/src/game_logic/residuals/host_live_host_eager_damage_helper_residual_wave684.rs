//! Wave 684 residual peels: post-logic damage apply on coupled tick.
//! Host records `host_damage_log` mid-frame; engine drains into GameWorld Damage
//! mutations immediately after the host frame. Session reuses the batch without
...
pub const LIVE_HOST_EAGER_DAMAGE_HELPER_METHOD_NAMES_WAVE684: &[&str] = &[
    "eager_apply_host_damage_after_logic",
    "host_damage_log",
    "take_early_damage_batch",
    "Wave 684",
    "playable_claim = false",
];
pub const LIVE_HOST_EAGER_DAMAGE_HELPER_NAV_STEPS_WAVE684: &[&str] = &[
    "REQUIRE_EAGER_DAMAGE_API",
    "REQUIRE_ENGINE_POST_LOGIC_DRAIN",
    "REQUIRE_SESSION_HANDOFF",
    "REQUIRE_NO_DOUBLE_APPLY",
    "LIVE_HOST_EAGER_DAMAGE_HELPER",
    "LIVE_PLAYABLE_CLAIM_FALSE",
];
pub const RUNTIME_HOST_LIVE_HOST_EAGER_DAMAGE_HELPER_CMD_NAMES_WAVE684: &[&str] = &[
    "host_eager_damage_helper",
    "engine_post_logic_drain",
    "session_handoff",
    "no_double_apply",
    "eager_damage_residual",
];
#[repr(u8)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ResidualHostEagerDamageHelperAction {
    None = 0,
    MethodNames = 1,
    SourceMarkers = 2,
    NavCommands = 3,
...
impl ResidualHostEagerDamageHelperAction {
...
fn residual_action_store(a: ResidualHostEagerDamageHelperAction) {
    RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst);
}
pub fn residual_host_eager_damage_helper_ok() -> bool {
    RESIDUAL_OK.load(Ordering::SeqCst)
}
pub fn residual_host_eager_damage_helper_last_action() -> ResidualHostEagerDamageHelperAction {
    ResidualHostEagerDamageHelperAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst))
}
...
pub fn honesty_host_eager_damage_helper_method_names_residual_wave684() -> bool {
    ...
    let ok = residual_name_index(names, "eager_apply_host_damage_after_logic").is_some()
        && residual_name_index(names, "host_damage_log").is_some()
        && residual_name_index(names, "take_early_damage_batch").is_some()
        && residual_name_index(names, "Wave 684").is_some()
        && residual_name_index(names, "playable_claim = false").is_some();
    residual_action_store(ResidualHostEagerDamageHelperAction::MethodNames);
    ok
}
pub fn honesty_host_eager_damage_helper_source_markers_residual_wave684() -> bool {
    let eng = eng_source();
    let sh = shadow_source();
    let api_ok = sh.contains("pub fn eager_apply_host_damage_after_logic")
        && sh.contains("Wave 684")
        && sh.contains("host_damage_log::drain")
...
pub fn honesty_host_eager_damage_helper_nav_commands_residual_wave684() -> bool {
    let steps = LIVE_HOST_EAGER_DAMAGE_HELPER_NAV_STEPS_WAVE684;
    let cmds = RUNTIME_HOST_LIVE_HOST_EAGER_DAMAGE_HELPER_CMD_NAMES_WAVE684;
    let ok = residual_name_index(steps, "REQUIRE_EAGER_DAMAGE_API").is_some()
        ...
        && residual_name_index(steps, "LIVE_HOST_EAGER_DAMAGE_HELPER").is_some()
        && residual_name_index(steps, "LIVE_PLAYABLE_CLAIM_FALSE").is_some()
        && residual_name_index(cmds, "host_eager_damage_helper").is_some()
        ...
        && residual_name_index(cmds, "eager_damage_residual").is_some();
    residual_action_store(ResidualHostEagerDamageHelperAction::NavCommands);
    ok
}
...
pub fn honesty_host_eager_damage_helper_residual_pack_wave684() -> bool {
    honesty_host_eager_damage_helper_method_names_residual_wave684()
        ...
        && simulate_host_eager_damage_helper_collect_source()
        && simulate_host_eager_damage_helper_dispatch_source()
}
pub fn simulate_live_host_eager_damage_helper_honesty() -> bool {
    let ok = honesty_host_eager_damage_helper_residual_pack_wave684();
    if ok {
        RESIDUAL_OK.store(true, Ordering::SeqCst);
        residual_action_store(ResidualHostEagerDamageHelperAction::Composite);
    }
    ok
}
...
mod tests {
    ...
    fn method_names_residual() {
        assert!(honesty_host_eager_damage_helper_method_names_residual_wave684());
    }
    #[test]
    fn source_markers_residual() {
        assert!(honesty_host_eager_damage_helper_source_markers_residual_wave684());
    }
    #[test]
    fn nav_commands_residual() {
        assert!(honesty_host_eager_damage_helper_nav_commands_residual_wave684());
    }
    #[test]
    fn sources() {
        assert!(simulate_host_eager_damage_helper_collect_source());
        assert!(simulate_host_eager_damage_helper_dispatch_source());
    }
    #[test]
    fn pack() {
        assert!(honesty_host_eager_damage_helper_residual_pack_wave684());
    }
    #[test]
    fn live() {
        assert!(simulate_live_host_eager_damage_helper_honesty());
        assert!(residual_host_eager_damage_helper_ok());
    }

    #[test]
    fn post_logic_damage_applies_once_through_session() {
        ...
        ensure_template(&mut logic, "EagerDmgUnit", 100.0);
        let id = logic
            .create_object("EagerDmgUnit", Team::USA, Vec3::new(0.0, 0.0, 0.0))
            .expect("spawn");
        // Under damage authority, take_damage may only log — force a log event.
        host_damage_log::record(id, 25.0, None, false);
        assert_eq!(host_damage_log::len(), 1);
        ...
        assert!(
            host_damage_log::drain().is_empty(),
