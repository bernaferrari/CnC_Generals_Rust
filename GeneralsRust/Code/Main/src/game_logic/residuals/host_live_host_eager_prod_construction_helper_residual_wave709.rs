//! Wave 709 residual peels: post-logic production + construction channels.
...
pub const LIVE_HOST_EAGER_PROD_CONSTRUCTION_HELPER_NAV_STEPS_WAVE709: &[&str] = &[
    "REQUIRE_EAGER_PROD_CONSTRUCTION_API",
    "REQUIRE_ENGINE_POST_LOGIC_DRAIN",
    "REQUIRE_SESSION_HANDOFF",
    "REQUIRE_NO_DOUBLE_APPLY",
    "LIVE_HOST_EAGER_PROD_CONSTRUCTION_HELPER",
    "LIVE_PLAYABLE_CLAIM_FALSE",
];
pub const RUNTIME_HOST_LIVE_HOST_EAGER_PROD_CONSTRUCTION_HELPER_CMD_NAMES_WAVE709: &[&str] = &[
    "host_eager_prod_construction_helper",
    "engine_post_logic_drain",
    "session_handoff",
    "no_double_apply",
    "eager_prod_construction_residual",
];
#[repr(u8)]
...
pub fn honesty_host_eager_prod_construction_helper_source_markers_residual_wave709() -> bool {
    ...
    let api_ok = sh.contains("pub fn eager_apply_host_production_after_logic")
        ...
        && sh.contains("Wave 709")
        && sh.contains("take_early_production_batch")
...
pub fn honesty_host_eager_prod_construction_helper_residual_pack_wave709() -> bool {
    honesty_host_eager_prod_construction_helper_method_names_residual_wave709()
        ...
        && simulate_host_eager_prod_construction_helper_collect_source()
...
mod tests {
    use super::*;
    use crate::game_logic::host_construction_log;
    ...
    fn post_logic_prod_construction_apply_once_through_session() {
        ...
        ensure_template(&mut logic, "EagerPcUnit", 100.0);
        ensure_template(&mut logic, "EagerPcChild", 50.0);
        let id = logic
            .create_object("EagerPcUnit", Team::USA, Vec3::new(0.0, 0.0, 0.0))
            .expect("spawn");
        host_production_log::clear();
        host_production_progress_log::clear();
        host_construction_log::clear();
        host_construction_progress_log::clear();
        host_production_log::record_enqueue(id, "EagerPcChild");
        ...
        host_construction_log::record(id, "EagerPcUnit");
        host_construction_progress_log::record(id, 0.5, true, 0.1);
        assert!(host_production_log::len() >= 1);
        assert_eq!(host_production_progress_log::len(), 1);
        assert_eq!(host_construction_log::len(), 1);
        assert_eq!(host_construction_progress_log::len(), 1);
        ...
        let _ = eager_apply_host_construction_progress_after_logic(&mut shadow, &logic);
        assert!(host_production_log::drain().is_empty());
        assert!(host_production_progress_log::drain().is_empty());
