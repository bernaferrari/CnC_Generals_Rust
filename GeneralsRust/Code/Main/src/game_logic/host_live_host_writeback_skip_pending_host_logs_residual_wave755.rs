...
pub enum ResidualHostWritebackSkipPendingHostLogsAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostWritebackSkipPendingHostLogsAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_writeback_skip_pending_host_logs_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_writeback_skip_pending_host_logs_last_action() -> ResidualHostWritebackSkipPendingHostLogsAction { ResidualHostWritebackSkipPendingHostLogsAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
fn sh_source() -> &'static str { include_str!("../gameworld_shadow.rs") }
fn cf_log() -> &'static str { include_str!("host_continuous_fire_log.rs") }
fn wb_log() -> &'static str { include_str!("host_weapon_bonus_log.rs") }
fn ff_log() -> &'static str { include_str!("host_faerie_fire_log.rs") }
pub fn honesty_host_writeback_skip_pending_host_logs_method_names_residual_wave755() -> bool {
    ...
    residual_action_store(ResidualHostWritebackSkipPendingHostLogsAction::MethodNames); ok
}
pub fn honesty_host_writeback_skip_pending_host_logs_source_markers_residual_wave755() -> bool {
    ...
    residual_action_store(ResidualHostWritebackSkipPendingHostLogsAction::SourceMarkers); ok
}
pub fn honesty_host_writeback_skip_pending_host_logs_nav_commands_residual_wave755() -> bool {
    ...
    residual_action_store(ResidualHostWritebackSkipPendingHostLogsAction::NavCommands); ok
}
pub fn simulate_host_writeback_skip_pending_host_logs_collect_source() -> bool {
    let ok=sh_source().contains("has_pending")
        && cf_log().contains("has_pending");
    residual_action_store(ResidualHostWritebackSkipPendingHostLogsAction::CollectSource); ok
}
pub fn simulate_host_writeback_skip_pending_host_logs_dispatch_source() -> bool {
    let ok=sh_source().contains("Wave 755")
        && sh_source().contains("mid-frame authority");
    residual_action_store(ResidualHostWritebackSkipPendingHostLogsAction::DispatchSource); ok
}
...
pub fn simulate_live_host_writeback_skip_pending_host_logs_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostWritebackSkipPendingHostLogsAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_writeback_skip_pending_host_logs_method_names_residual_wave755()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_writeback_skip_pending_host_logs_source_markers_residual_wave755()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_writeback_skip_pending_host_logs_nav_commands_residual_wave755()); }
    #[test] fn sources() { assert!(simulate_host_writeback_skip_pending_host_logs_collect_source()); assert!(simulate_host_writeback_skip_pending_host_logs_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_writeback_skip_pending_host_logs_residual_pack_wave755()); }
    #[test] fn live() { assert!(simulate_live_host_writeback_skip_pending_host_logs_honesty()); assert!(residual_host_writeback_skip_pending_host_logs_ok()); }
}
