...
pub enum ResidualHostEnsureBarracksOptInAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostEnsureBarracksOptInAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_ensure_barracks_opt_in_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_ensure_barracks_opt_in_last_action() -> ResidualHostEnsureBarracksOptInAction { ResidualHostEnsureBarracksOptInAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
...
pub fn honesty_host_ensure_barracks_opt_in_method_names_residual_wave723() -> bool {
    ...
    residual_action_store(ResidualHostEnsureBarracksOptInAction::MethodNames); ok
}
pub fn honesty_host_ensure_barracks_opt_in_source_markers_residual_wave723() -> bool {
    ...
    let no_uncond = eng.matches("let _ = self.host_ensure_barracks_building_data(id);").count() == 1;
    ...
    let ok=eng_ok&&gated&&no_uncond&&force_implies&&smoke_ok&&!eng.contains("playable_claim = true");
    residual_action_store(ResidualHostEnsureBarracksOptInAction::SourceMarkers); ok
}
pub fn honesty_host_ensure_barracks_opt_in_nav_commands_residual_wave723() -> bool {
    ...
    residual_action_store(ResidualHostEnsureBarracksOptInAction::NavCommands); ok
}
pub fn simulate_host_ensure_barracks_opt_in_collect_source() -> bool {
    ...
    residual_action_store(ResidualHostEnsureBarracksOptInAction::CollectSource); ok
}
pub fn simulate_host_ensure_barracks_opt_in_dispatch_source() -> bool {
    let ok=eng_source().contains("Wave 723")
        && smoke_source().contains("force_complete=1");
    residual_action_store(ResidualHostEnsureBarracksOptInAction::DispatchSource); ok
}
...
pub fn simulate_live_host_ensure_barracks_opt_in_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostEnsureBarracksOptInAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_ensure_barracks_opt_in_method_names_residual_wave723()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_ensure_barracks_opt_in_source_markers_residual_wave723()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_ensure_barracks_opt_in_nav_commands_residual_wave723()); }
    #[test] fn sources() { assert!(simulate_host_ensure_barracks_opt_in_collect_source()); assert!(simulate_host_ensure_barracks_opt_in_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_ensure_barracks_opt_in_residual_pack_wave723()); }
    #[test] fn live() { assert!(simulate_live_host_ensure_barracks_opt_in_honesty()); assert!(residual_host_ensure_barracks_opt_in_ok()); }
}
