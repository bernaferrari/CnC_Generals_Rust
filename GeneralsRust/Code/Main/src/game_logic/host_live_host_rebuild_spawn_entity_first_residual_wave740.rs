...
pub enum ResidualHostRebuildSpawnEntityFirstAction { None=0,MethodNames=1,SourceMarkers=2,NavCommands=3,CollectSource=4,DispatchSource=5,Composite=6 }
...
fn residual_action_store(a: ResidualHostRebuildSpawnEntityFirstAction) { RESIDUAL_ACTION.store(a as u8, Ordering::SeqCst); }
pub fn residual_host_rebuild_spawn_entity_first_ok() -> bool { RESIDUAL_OK.load(Ordering::SeqCst) }
pub fn residual_host_rebuild_spawn_entity_first_last_action() -> ResidualHostRebuildSpawnEntityFirstAction { ResidualHostRebuildSpawnEntityFirstAction::from_u8(RESIDUAL_ACTION.load(Ordering::SeqCst)) }
fn gl_source() -> &'static str { include_str!("game_logic.rs") }
fn ready_source() -> &'static str { include_str!("host_rebuild_ready_log.rs") }
...
pub fn honesty_host_rebuild_spawn_entity_first_method_names_residual_wave740() -> bool {
    ...
    residual_action_store(ResidualHostRebuildSpawnEntityFirstAction::MethodNames); ok
}
pub fn honesty_host_rebuild_spawn_entity_first_source_markers_residual_wave740() -> bool {
    ...
    residual_action_store(ResidualHostRebuildSpawnEntityFirstAction::SourceMarkers); ok
}
pub fn honesty_host_rebuild_spawn_entity_first_nav_commands_residual_wave740() -> bool {
    ...
    residual_action_store(ResidualHostRebuildSpawnEntityFirstAction::NavCommands); ok
}
pub fn simulate_host_rebuild_spawn_entity_first_collect_source() -> bool {
    let ok=ready_source().contains("record_with_entities")
        && gl_source().contains("ready_by_hole");
    residual_action_store(ResidualHostRebuildSpawnEntityFirstAction::CollectSource); ok
}
pub fn simulate_host_rebuild_spawn_entity_first_dispatch_source() -> bool {
    ...
    residual_action_store(ResidualHostRebuildSpawnEntityFirstAction::DispatchSource); ok
}
...
pub fn simulate_live_host_rebuild_spawn_entity_first_honesty() -> bool {
    ...
    if ok { RESIDUAL_OK.store(true, Ordering::SeqCst); residual_action_store(ResidualHostRebuildSpawnEntityFirstAction::Composite); }
    ok
}
...
mod tests {
    use super::*;
    #[test] fn method_names_residual() { assert!(honesty_host_rebuild_spawn_entity_first_method_names_residual_wave740()); }
    #[test] fn source_markers_residual() { assert!(honesty_host_rebuild_spawn_entity_first_source_markers_residual_wave740()); }
    #[test] fn nav_commands_residual() { assert!(honesty_host_rebuild_spawn_entity_first_nav_commands_residual_wave740()); }
    #[test] fn sources() { assert!(simulate_host_rebuild_spawn_entity_first_collect_source()); assert!(simulate_host_rebuild_spawn_entity_first_dispatch_source()); }
    #[test] fn pack() { assert!(honesty_host_rebuild_spawn_entity_first_residual_pack_wave740()); }
    #[test] fn live() { assert!(simulate_live_host_rebuild_spawn_entity_first_honesty()); assert!(residual_host_rebuild_spawn_entity_first_ok()); }
}
