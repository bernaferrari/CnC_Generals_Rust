...
fn test_framerate(
    ...
    println!(
        "\n🎯 Testing {} (Logic: 30Hz, Render: {:.0}Hz)",
        test_name, render_fps
    );
    ...
    while render_time < TEST_DURATION {
        ...
        if let Some(interpolated_pos) = interpolation_manager.get_interpolated_position(object_id) {
            ...
            if render_frames % 10 == 0 {
                println!(
                    "   Frame {}: alpha={:.3}, pos=({:.1}, {:.1}, {:.1}), error={:.3}",
                    render_frames,
                    alpha,
                    interpolated_pos.x,
                    interpolated_pos.y,
    ...
    println!(
        "   Completed {} render frames in {:.1}s",
        render_frames, TEST_DURATION
    );
...
fn test_camera_interpolation(interpolation_manager: &mut InterpolationManager) {
    ...
    interpolation_manager
        .camera_state
        .update(initial_pos, initial_target, initial_zoom);
    interpolation_manager
        .camera_state
        .update(target_pos, target_target, target_zoom);
    ...
    for alpha in test_alphas {
        ...
        println!(
            "   Alpha {:.2}: pos_error={:.3}, target_error={:.3}, zoom_error={:.3}",
...
fn test_alpha_clamping(interpolation_manager: &mut InterpolationManager, object_id: ObjectId) {
    ...
    for alpha in test_alphas {
        ...
        if let Some(interpolated_pos) = interpolation_manager.get_interpolated_position(object_id) {
            ...
            println!(
                "   Alpha {:.2} -> Clamped {:.2}: pos=({:.1}, {:.1}, {:.1}), error={:.3}",
                alpha,
                clamped_alpha,
                interpolated_pos.x,
                interpolated_pos.y,
            ...
            if alpha < 0.0 {
                assert!(
                    (interpolated_pos.distance(initial_pos) < 0.01),
                    "Extrapolation occurred below 0.0"
                );
            } else if alpha > 1.0 {
                assert!(
                    (interpolated_pos.distance(target_pos) < 0.01),
                    "Extrapolation occurred above 1.0"
                );
            }
        }
    }
