...
pub use super::*;
    ...
    fn render_execute_passes_none_game_logic_when_presentation_installed() {
        ...
        let rp = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/graphics/render_pipeline.rs"));
        assert!(
            rp.contains(
    ...
    fn defeat_alliance_prefer_presentation_no_live_dual_read_when_frame_installed() {
        let src = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/cnc_game_engine.rs"));
        assert!(
            src.contains(
