#!/usr/bin/env python3
"""Generate legacy filename-hint matrices plus the authoritative provenance manifest.

The text matrices are deliberately non-authoritative: they use filename/stem
heuristics, cannot represent complete split-module ownership, and never prove
behavior. `generate_port_provenance.py` owns all parity grades and blockers.
"""
...
MANUAL_CPP_TO_RUST: dict[tuple[str, str], str] = {
    ...
    ): "GameClient/src/drawable/drawable/mod.rs",
    (
        "Source",
        "GameClient/Terrain/TerrainVisual.cpp",
    ): "GameClient/src/terrain/terrain_visual/mod.rs",
    (
        "Source",
        "GameLogic/AI/AI.cpp",
    ...
    ): "GameLogic/src/ai/ai_group/mod.rs",
    (
        "Source",
        "Common/Audio/GameAudio.cpp",
    ): "Common/src/common/audio/game_audio.rs",
    (
        "Include",
...
def write_matrix(path: Path, rows: Iterable[MappingRow]) -> None:
    with path.open("w", encoding="utf-8") as handle:
        ...
        handle.write("# LEGACY FILENAME HINTS ONLY; not parity or completion evidence.\n")
        handle.write("# Authoritative source: PORT_PROVENANCE_MANIFEST.json\n")
...
def write_missing(path: Path, rows: Iterable[MappingRow]) -> None:
    ...
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# Auto-generated by generate_port_tracking.py\n")
        handle.write("# LEGACY FILENAME HINTS ONLY; split-aware blockers live in PORT_PROVENANCE_MANIFEST.json\n")
...
def write_state(path: Path, rows: Iterable[MappingRow]) -> None:
    ...
    with path.open("w", encoding="utf-8") as handle:
        handle.write("# Auto-generated by generate_port_tracking.py\n")
        handle.write("# Metric=stem-existence only; playable_claim=false\n")
        handle.write(
            "# This is NOT behavioral parity and NOT a playable-game claim.\n"
        )
        ...
        handle.write(f"StemExistencePercent={parity:.2f}\n")
...
def main() -> None:
    ...
    # Legacy compatibility matrix: never allow a test/example file to satisfy a
    # production C++ source/header mapping. The repository-wide provenance
    # manifest performs stronger Cargo reachability and classification checks.
    rust_files = [
        path
        for path in collect_files(rust_root, (".rs",))
        if not any(part.lower() in {"tests", "examples", "benches"} for part in path.parts)
        and path.name.lower() not in {"test.rs", "tests.rs", "parity_tests.rs"}
    ]
    ...
    # The legacy matrix above remains stable for existing Rust consumers. The
    # repository-wide manifest overwrites the two headline status artifacts with
    # truthful full-scope metrics and adds one-to-many provenance records.
    from generate_port_provenance import generate as generate_provenance

    provenance = generate_provenance(repo_root, output_root)
    provenance_path = output_root / "PORT_PROVENANCE_MANIFEST.json"
    from generate_port_review_queue import (
        DEFAULT_PACKET_SIZE,
        OUTPUT_NAME,
        generate as generate_review_queue,
    )

    review_queue_path = output_root / OUTPUT_NAME
    review_queue = generate_review_queue(
        repo_root, review_queue_path, DEFAULT_PACKET_SIZE, provenance
    )
    ...
    print(f"Generated: {review_queue_path}")
    ...
    print(
        ...
        f"packets={review_queue['summary']['packets']} "
        f"unique_units={review_queue['summary']['unique_units']}"
    )
