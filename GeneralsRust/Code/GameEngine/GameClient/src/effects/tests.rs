...
mod particle_system_tests {
    ...
    fn test_particle_creation_and_lifecycle() {
        let info = ParticleInfo {
            ...
            size: 2.0,
            alpha_keys: [
                Keyframe { value: 1.0, frame: 0 },
                Keyframe { value: 0.5, frame: 30 },
                Keyframe { value: 0.0, frame: 60 },
                Keyframe::default(),
                Keyframe::default(),
                Keyframe::default(),
                Keyframe::default(),
    ...
    fn test_particle_system_attachment() {
        ...
        let object_id = 12345;
        let system_id = manager.create_attached_particle_system(&template, object_id, false).unwrap();
    ...
    fn test_keyframe_interpolation() {
        let info = ParticleInfo {
            ...
            color_keys: [
                RGBColorKeyframe { color: [1.0, 0.0, 0.0], frame: 0 }, // Red
                RGBColorKeyframe { color: [0.0, 1.0, 0.0], frame: 30 }, // Green
                RGBColorKeyframe { color: [0.0, 0.0, 1.0], frame: 60 }, // Blue
...
mod particle_optimization_tests {
    ...
    fn test_lod_multipliers() {
        ...
        assert_eq!(optimizer.get_lod_particle_multiplier(ParticleLODLevel::High), 1.0);
        assert_eq!(optimizer.get_lod_particle_multiplier(ParticleLODLevel::Medium), 0.6);
        assert_eq!(optimizer.get_lod_particle_multiplier(ParticleLODLevel::Low), 0.3);
        assert_eq!(optimizer.get_lod_particle_multiplier(ParticleLODLevel::Minimal), 0.1);
        assert_eq!(optimizer.get_lod_particle_multiplier(ParticleLODLevel::Culled), 0.0);
        
        // Test update multipliers
        assert_eq!(optimizer.get_lod_update_multiplier(ParticleLODLevel::High), 1.0);
        assert_eq!(optimizer.get_lod_update_multiplier(ParticleLODLevel::Medium), 0.8);
        assert_eq!(optimizer.get_lod_update_multiplier(ParticleLODLevel::Low), 0.5);
        assert_eq!(optimizer.get_lod_update_multiplier(ParticleLODLevel::Minimal), 0.25);
        assert_eq!(optimizer.get_lod_update_multiplier(ParticleLODLevel::Culled), 0.0);
    }
    ...
    fn test_performance_budget() {
        ...
        let critical_budget = budget.max_particles_by_priority.get(&ParticlePriorityType::Critical).unwrap();
        let explosion_budget = budget.max_particles_by_priority.get(&ParticlePriorityType::WeaponExplosion).unwrap();
...
mod particle_ini_tests {
    ...
    fn test_enum_parsing() {
        ...
        assert_eq!(parser.parse_shader_type("ALPHA").unwrap(), ParticleShaderType::Alpha);
        assert_eq!(parser.parse_shader_type("ALPHA_TEST").unwrap(), ParticleShaderType::AlphaTest);
        assert_eq!(parser.parse_shader_type("MULTIPLY").unwrap(), ParticleShaderType::Multiply);
        assert!(parser.parse_shader_type("INVALID").is_err());
        
        // Test particle types
        assert_eq!(parser.parse_particle_type("PARTICLE").unwrap(), NewParticleType::Particle);
        assert_eq!(parser.parse_particle_type("DRAWABLE").unwrap(), NewParticleType::Drawable);
        assert_eq!(parser.parse_particle_type("STREAK").unwrap(), NewParticleType::Streak);
        assert_eq!(parser.parse_particle_type("VOLUME_PARTICLE").unwrap(), NewParticleType::VolumeParticle);
        assert_eq!(parser.parse_particle_type("SMUDGE").unwrap(), NewParticleType::Smudge);
        assert!(parser.parse_particle_type("INVALID").is_err());
        
        // Test priorities
        assert_eq!(parser.parse_priority("ALWAYS_RENDER").unwrap(), ParticlePriorityType::AlwaysRender);
        assert_eq!(parser.parse_priority("CRITICAL").unwrap(), ParticlePriorityType::Critical);
        assert_eq!(parser.parse_priority("WEAPON_EXPLOSION").unwrap(), ParticlePriorityType::WeaponExplosion);
        assert!(parser.parse_priority("INVALID").is_err());
        
        // Test emission velocity types
        assert_eq!(parser.parse_emission_velocity_type("ORTHO").unwrap(), EmissionVelocityType::Ortho);
        assert_eq!(parser.parse_emission_velocity_type("SPHERICAL").unwrap(), EmissionVelocityType::Spherical);
        assert_eq!(parser.parse_emission_velocity_type("HEMISPHERICAL").unwrap(), EmissionVelocityType::Hemispherical);
        assert_eq!(parser.parse_emission_velocity_type("CYLINDRICAL").unwrap(), EmissionVelocityType::Cylindrical);
        assert_eq!(parser.parse_emission_velocity_type("OUTWARD").unwrap(), EmissionVelocityType::Outward);
        assert!(parser.parse_emission_velocity_type("INVALID").is_err());
        
        // Test emission volume types
        assert_eq!(parser.parse_emission_volume_type("POINT").unwrap(), EmissionVolumeType::Point);
        assert_eq!(parser.parse_emission_volume_type("LINE").unwrap(), EmissionVolumeType::Line);
        assert_eq!(parser.parse_emission_volume_type("BOX").unwrap(), EmissionVolumeType::Box);
        assert_eq!(parser.parse_emission_volume_type("SPHERE").unwrap(), EmissionVolumeType::Sphere);
        assert_eq!(parser.parse_emission_volume_type("CYLINDER").unwrap(), EmissionVolumeType::Cylinder);
        assert!(parser.parse_emission_volume_type("INVALID").is_err());
        
        // Test wind motion
        assert_eq!(parser.parse_wind_motion("Unused").unwrap(), WindMotion::NotUsed);
        assert_eq!(parser.parse_wind_motion("PingPong").unwrap(), WindMotion::PingPong);
        assert_eq!(parser.parse_wind_motion("Circular").unwrap(), WindMotion::Circular);
...
mod particle_renderer_tests {
    ...
    fn test_particle_batch() {
        let mut batch = ParticleBatch::new(ParticleShaderType::Alpha, "test_texture.tga".to_string());
...
mod integration_tests {
    ...
    fn test_complete_explosion_effect() {
        ...
        let template = manager.new_template("BigExplosion".to_string());
        {
            ...
            info.alpha_keys[0] = RandomKeyframe { min_value: 1.0, max_value: 1.0, frame: 0 };
            info.alpha_keys[1] = RandomKeyframe { min_value: 0.8, max_value: 0.8, frame: 15 };
            info.alpha_keys[2] = RandomKeyframe { min_value: 0.0, max_value: 0.0, frame: 60 };
            
            // Color animation (fire colors)
            info.color_keys[0] = RGBColorKeyframe { color: [1.0, 1.0, 0.8], frame: 0 }; // Bright yellow
            info.color_keys[1] = RGBColorKeyframe { color: [1.0, 0.3, 0.0], frame: 20 }; // Orange
            info.color_keys[2] = RGBColorKeyframe { color: [0.5, 0.0, 0.0], frame: 60 }; // Dark red
