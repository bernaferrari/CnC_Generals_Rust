...
impl ParticleOptimizer {
    ...
    pub fn should_cull_system(
        ...
        if system
            .particles()
            .iter()
            .all(|p| p.is_invisible(info.shader_type))
        {
            return true;
        }
    ...
    fn adjust_quality_for_performance(&mut self) {
        ...
        let avg_particle_count: f64 =
            self.particle_counts.iter().sum::<usize>() as f64 / self.particle_counts.len() as f64;
        ...
        if avg_frame_time > target_frame_time * 1.2 || self.performance_trend > 0.5 {
            ...
            self.lod_settings.near_distance = self
                .lod_settings
                .near_distance
                .max(default_settings.near_distance * 1.01);
            self.lod_settings.medium_distance = self
                .lod_settings
                .medium_distance
                .max(default_settings.medium_distance * 1.01);
            self.lod_settings.far_distance = self
                .lod_settings
                .far_distance
                .max(default_settings.far_distance * 1.01);
        }
