...
impl ReplayGameInfo {
    pub fn new() -> Self {
        ReplayGameInfo {
            replay_slots: [
                GameSlot::new(), GameSlot::new(), GameSlot::new(), GameSlot::new(),
                GameSlot::new(), GameSlot::new(), GameSlot::new(), GameSlot::new(),
            ],
            map_name: String::new(),
            game_mode: 0,
...
impl ReplayHeader {
    ...
    pub fn is_version_compatible(&self, current_version: &str, current_version_num: u32, current_exe_crc: u32, current_ini_crc: u32) -> bool {
    ...
    pub fn get_display_color(&self, current_version: &str, current_version_num: u32, current_exe_crc: u32, current_ini_crc: u32) -> Color {
        if self.is_version_compatible(current_version, current_version_num, current_exe_crc, current_ini_crc) {
            // Good version
...
impl ReplayListEntry {
    pub fn new(name: String, date: String, version: String, map: String, color: Color, filename: String) -> Self {
        ReplayListEntry {
            name,
            date,
            version,
...
impl ReplayMenu {
        ...
        for filepath in replay_files {
            ...
            if let Some(header) = self.read_replay_header(&filepath) {
                ...
                if let Some(game_info) = self.parse_game_info(&header.game_options) {
                    ...
                    let last_replay_full = format!("{}{}", self.last_replay_filename, self.replay_extension);
    ...
    fn parse_game_info(&self, _options: &str) -> Option<ReplayGameInfo> {
        // In actual implementation, parse the game options string
        // For now, return a dummy game info
        Some(ReplayGameInfo::new())
    }
...
pub fn parse_ascii_string_to_game_info(game_info: &mut ReplayGameInfo, options: &str) -> bool {
    let mut net_info = game_info::GameInfo::new();
    if !game_info::serialization::parse_ascii_string_to_game_info(&mut net_info, options) {
        return false;
    }
...
fn map_slot_state(state: game_info::SlotState) -> SlotState {
    match state {
        game_info::SlotState::Open => SlotState::Open,
        game_info::SlotState::Closed => SlotState::Closed,
        game_info::SlotState::EasyAI => SlotState::EasyAI,
        game_info::SlotState::MedAI => SlotState::MedAI,
        game_info::SlotState::BrutalAI => SlotState::BrutalAI,
        game_info::SlotState::Player => SlotState::Player,
    }
}
