...
// Additional example showing game loop integration
#[allow(dead_code)]
fn game_loop_example() {
    ...
    loop {
        // Update game logic
        // ... (units move, attack, build, etc.)

        // Update scores
        // game_state.score_keeper.add_score(player_idx, ScoreCategory::UnitsBuilt, 1);

        // Check victory conditions
        let game_ended = game_state.victory_conditions.check_conditions(
            &game_state.player_list,
            &game_state.score_keeper,
        );

        if game_ended {
            // Game over - display results
            for player in game_state.player_list.get_all_players() {
                let result = game_state.victory_conditions.get_player_result(player.index);
                match result {
                    GameResult::Victory => println!("{} WINS!", player.name),
                    GameResult::Defeat => println!("{} defeated", player.name),
                    _ => {}
                }
            }
            break;
