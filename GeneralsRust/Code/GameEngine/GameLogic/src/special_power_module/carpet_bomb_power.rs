//! Carpet Bomb Special Power
//!
//! China special power that drops a line of bombs across the target area.
...
#[derive(Debug, Clone)]
pub struct CarpetBombPowerData {
    pub base: SpecialPowerModuleData,
    pub bomb_count: Int,
    pub bomb_spacing: Real,
...
impl CarpetBombPowerData {
    pub fn new(name: AsciiString) -> Self {
        let mut base = SpecialPowerModuleData::new(name, SpecialPowerKind::CarpetBomb);
        ...
        if name_str.eq_ignore_ascii_case("SuperweaponChinaCarpetBomb")
            || name_str.eq_ignore_ascii_case("Early_SuperweaponChinaCarpetBomb")
        {
            base.radius = 180.0;
        } else if name_str.eq_ignore_ascii_case("Nuke_SuperweaponChinaCarpetBomb") {
            base.recharge_time = 180.0; // 180000 ms
            base.radius = 180.0;
        } else if name_str.eq_ignore_ascii_case("AirF_SuperweaponCarpetBomb") {
        ...
        Self {
            base,
            bomb_count: CARPET_BOMB_COUNT,
            bomb_spacing: CARPET_BOMB_SPACING,
            bomber_height: 350.0,
            bomb_damage: 400.0,
            blast_radius: 50.0,
            bomber_template: "ChinaJetCarpetBomber".into(),
            ocl_name: "SUPERWEAPON_CarpetBomb".into(),
            upgrade_ocl: Vec::new(),
...
pub struct CarpetBombPower {
    data: CarpetBombPowerData,
    cooldown: CooldownState,
    stats: SpecialPowerStats,
    bomber_id: Option<ObjectID>,
    bombs_dropped: Vec<ObjectID>,
    owner_player_id: Option<ObjectID>,
    owner_object_id: ObjectID,
}

impl CarpetBombPower {
    pub fn new(data: CarpetBombPowerData) -> Self {
    ...
    fn validate_targeting(&self, targeting: Option<&TargetingInfo>) -> Result<(), String> {
        if self.data.base.requires_targeting() && targeting.is_none() {
            return Err("Carpet Bomb requires targeting".to_string());
        }
    ...
    fn execute_strike(
        ...
        self.bombs_dropped.clear();

        let owner = self
            .resolve_owner_object()
            .ok_or_else(|| "Carpet Bomb requires an owning object".to_string())?;
        let owner_guard = owner
            .read()
            .map_err(|_| "Carpet bomb owner lock poisoned".to_string())?;
        ...
        let ocl_name = self
            .select_ocl_name()
            .ok_or_else(|| "Carpet Bomb has no OCL configured".to_string())?;
        let ocl = TheObjectCreationListStore::find_object_creation_list(ocl_name.as_str())
            .ok_or_else(|| format!("OCL '{}' not found for carpet bomb", ocl_name))?;
        ...
        if let Some(obj) = created {
            if let Ok(guard) = obj.read() {
                self.bombs_dropped.push(guard.get_id());
            }
        }
...
impl SpecialPowerModuleInterface for CarpetBombPower {
        ...
        let targeting = match targeting {
            ...
            None => {
                return ActivationResult::InvalidTarget {
                    reason: "Carpet Bomb requires targeting".to_string(),
                };
            }
        };
    ...
    fn execute(&mut self, targeting: &TargetingInfo) -> Result<(), String> {
        let player_id = self
            .owner_player_id
            .ok_or_else(|| "Carpet Bomb requires an owning player".to_string())?;
