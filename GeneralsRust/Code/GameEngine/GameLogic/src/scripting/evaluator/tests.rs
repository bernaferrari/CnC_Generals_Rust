    ...
    fn team_reached_waypoints_end_requires_the_requested_path_like_cpp() {
        ...
        let completed_waypoint_id = Arc::new(std::sync::atomic::AtomicU32::new(
            OTHER_PATH_WAYPOINT_ID,
        ));
        let object = Arc::new(RwLock::new(crate::object::Object::new_test(MEMBER_ID, 100.0)));
        fixture.object = Some(Arc::clone(&object));
        let ai: Arc<std::sync::Mutex<dyn crate::modules::AIUpdateInterface>> = Arc::new(
            std::sync::Mutex::new(CompletedWaypointAi {
        ...
        object
            .write()
            .unwrap()
            .set_ai_update_interface(Some(ai));
    ...
    fn enemy_and_type_sighted_honor_cxx_relation_and_stealth_filters() {
        ...
        let source = Arc::new(RwLock::new(crate::object::Object::new_test(SOURCE_ID, 100.0)));
        ...
        source_team
            .write()
            .unwrap()
            .set_override_team_relationship(
                CANDIDATE_TEAM_ID,
