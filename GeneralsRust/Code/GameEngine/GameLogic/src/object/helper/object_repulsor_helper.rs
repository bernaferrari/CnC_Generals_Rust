...
impl Snapshotable for ObjectRepulsorHelper {
    fn crc(&self, xfer: &mut dyn Xfer) -> Result<(), String> {
        ...
        xfer.xfer_version(&mut version, CURRENT_VERSION)
            .map_err(|err| format!("ObjectRepulsorHelper crc version: {err:?}"))?;

        let mut object_helper_version = CURRENT_VERSION;
        xfer.xfer_version(&mut object_helper_version, CURRENT_VERSION)
            .map_err(|err| format!("ObjectRepulsorHelper crc object helper version: {err:?}"))?;
        let mut next_call_frame_and_phase = self.next_call_frame_and_phase;
        xfer_update_module_base_state(xfer, &mut next_call_frame_and_phase)
            .map_err(|err| format!("ObjectRepulsorHelper crc update module base: {err}"))?;
