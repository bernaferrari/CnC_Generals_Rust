...
impl W3DOverlordTruckDrawModuleData {
    pub fn new() -> Self {
        Self {
...
impl ModuleData for W3DOverlordTruckDrawModuleData {
...
impl DrawModuleData for W3DOverlordTruckDrawModuleData {
    fn as_any(&self) -> &dyn Any {
        self
    }
}
impl Snapshotable for W3DOverlordTruckDrawModuleData {
...
pub struct W3DOverlordTruckDraw {
    data: W3DOverlordTruckDrawModuleData,
    base: W3DTruckDraw,
    owner_id: Option<ObjectID>,
}
impl W3DOverlordTruckDraw {
    pub fn new(data: W3DOverlordTruckDrawModuleData) -> Self {
        Self {
...
impl DrawModule for W3DOverlordTruckDraw {
    fn do_draw_module(&mut self, transform_mtx: &Matrix3D) {
        ...
        let drawable = drawable.clone();
        {
            ...
            draw_guard.set_color_tint(tint);
            draw_guard.set_tint_status_exact(tint_status);
            draw_guard.notify_drawable_dependency_cleared();
            draw_guard.draw(None);
        }
    }
...
impl Snapshotable for W3DOverlordTruckDraw {
    fn crc(&self, xfer: &mut dyn Xfer) -> Result<(), String> {
        self.base.crc(xfer)
    }
    fn xfer(&mut self, xfer: &mut dyn Xfer) -> Result<(), String> {
...
mod tests {
    ...
    fn bind_owner_id_sets_wrapper_owner() {
        let mut draw = W3DOverlordTruckDraw::new(W3DOverlordTruckDrawModuleData::new());
        draw.bind_owner_id(88);
        assert_eq!(draw.owner_id(), Some(88));
    }

    #[test]
    fn module_name_key_is_overlord_truck_draw() {
        let draw = W3DOverlordTruckDraw::new(W3DOverlordTruckDrawModuleData::new());
        assert_eq!(
            draw.get_module_name_key(),
            NameKeyGenerator::name_to_key("W3DOverlordTruckDraw")
        );
    }
}
