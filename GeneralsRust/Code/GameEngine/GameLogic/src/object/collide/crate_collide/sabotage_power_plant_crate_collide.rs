...
impl LegacyCollideAdapter for SabotagePowerPlantCrateCollide {
    fn legacy_on_collide(
        ...
        if SabotagePowerPlantCrateCollide::is_valid_to_execute(
            self,
            other.read().map(|g| g.get_id()).unwrap_or(0),
        )? {
            let success = SabotagePowerPlantCrateCollide::execute_crate_behavior(
                self,
                other.read().map(|g| g.get_id()).unwrap_or(0),
            )?;
            self.base
    ...
    fn legacy_would_like_to_collide_with(
        ...
        SabotagePowerPlantCrateCollide::is_valid_to_execute(
            self,
            other.read().map(|g| g.get_id()).unwrap_or(0),
        )
    }
...
impl CrateCollideModule for SabotagePowerPlantCrateCollide {
    ...
    fn execute_crate_behavior(&mut self, other_id: ObjectID) -> Result<bool, GameError> {
        ...
        SabotagePowerPlantCrateCollide::execute_crate_behavior(
            self,
            other.read().map(|g| g.get_id()).unwrap_or(0),
        )
    }
