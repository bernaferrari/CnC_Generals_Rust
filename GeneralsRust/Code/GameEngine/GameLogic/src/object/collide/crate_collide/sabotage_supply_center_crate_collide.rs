...
impl SabotageSupplyCenterCrateCollideModuleData {
    pub fn parse_from_ini(&mut self, ini: &mut INI) -> Result<(), INIError> {
        ini.init_from_ini_with_fields(self, SABOTAGE_SUPPLY_CENTER_CRATE_COLLIDE_FIELDS)
    }
...
fn parse_required_kind_of(
    _ini: &mut INI,
    data: &mut SabotageSupplyCenterCrateCollideModuleData,
    tokens: &[&str],
) -> Result<(), INIError> {
...
const SABOTAGE_SUPPLY_CENTER_CRATE_COLLIDE_FIELDS: &[IniFieldParse<
...
impl LegacyCollideAdapter for SabotageSupplyCenterCrateCollide {
    fn legacy_on_collide(
        ...
        if SabotageSupplyCenterCrateCollide::is_valid_to_execute(self, other_id)? {
            let success = SabotageSupplyCenterCrateCollide::execute_crate_behavior(self, other_id)?;
    ...
    fn legacy_would_like_to_collide_with(
        ...
        SabotageSupplyCenterCrateCollide::is_valid_to_execute(self, other_id)
    }
...
impl CrateCollideModule for SabotageSupplyCenterCrateCollide {
    ...
    fn execute_crate_behavior(&mut self, other_id: ObjectID) -> Result<bool, GameError> {
        ...
        SabotageSupplyCenterCrateCollide::execute_crate_behavior(self, other_id)
    }
...
mod tests {
    ...
    fn steal_cash_amount_parse_from_ini_preserves_cpp_field() {
        ...
        ini.with_inline_source(
            "StealCashAmount = 1500\n\
             RequiredKindOf = FS_SUPPLY_CENTER\n\
             End\n",
            |ini| data.parse_from_ini(ini),
        )
        .expect("sabotage supply center ini parses");

        assert_eq!(data.steal_cash_amount, 1500);
        assert_ne!(
            data.base.required_kind_of & (1u64 << (KindOf::FSSupplyCenter as u32)),
            0
        );
    }
    ...
    fn steal_cash_without_player_handles_is_best_effort_like_cpp() {
        ...
        let saboteur = Arc::new(RwLock::new(Object::new_test(58_101, 100.0)));
        let target = Arc::new(RwLock::new(Object::new_test(58_102, 100.0)));
        let module_data = SabotageSupplyCenterCrateCollideModuleData {
