...
impl SpatialNode {
    ...
    pub fn subdivide(&mut self) {
        ...
        for i in 0..8 {
            let x_offset = if i & 1 != 0 {
                half_extents.x
            } else {
                -half_extents.x
            };
            let y_offset = if i & 2 != 0 {
                half_extents.y
            } else {
                -half_extents.y
            };
            let z_offset = if i & 4 != 0 {
                half_extents.z
            } else {
                -half_extents.z
            };
    ...
    pub fn insert_object(&mut self, object_key: DefaultKey, bounds: AABB) {
        if let Some(ref mut children) = self.children {
            ...
            if self.should_subdivide() {
                ...
                // PARITY_NOTE: C++ W3DTreeBuffer.cpp does NOT use octree subdivision.
                // It uses a flat array with area partition buckets (PARTITION_WIDTH_HEIGHT grid)
                // and getPartitionBucket() for spatial lookup. Trees are stored in m_trees[]
                // with bounds, visible flag, sortKey, swayType. No redistribution needed.
                // This Rust implementation uses an octree which needs redistribution:
                // after subdivide(), each object should be inserted into the child whose
                // bounds contain the object's center position.
                let objects_to_redistribute = std::mem::take(&mut self.objects);
                if let Some(ref children) = self.children {
                    for obj_key in &objects_to_redistribute {
                        for child in children.iter() {
                            // Insert into first child whose bounds contain the object
                            // (requires object position lookup from the tree buffer)
                            child.insert(*obj_key);
                            break;
                        }
                    }
                }
...
impl W3DTreeBuffer {
    ...
    fn create_initial_buffers(&mut self) -> Result<()> {
        let device = self
            .device
            .as_ref()
            .ok_or(TreeBufferError::DeviceNotAvailable)?;
    ...
    pub fn add_object(&mut self, mut object: RenderObject) -> Result<DefaultKey> {
        ...
        self.root_node
            .insert_object(object_key, object.world_bounds);
    ...
    fn update_object_world_bounds(&mut self, object: &mut RenderObject) {
        ...
        let corners = [
            Point3::new(
                object.local_bounds.min.x,
                object.local_bounds.min.y,
                object.local_bounds.min.z,
            ),
            Point3::new(
                object.local_bounds.max.x,
                object.local_bounds.min.y,
                object.local_bounds.min.z,
            ),
            Point3::new(
                object.local_bounds.min.x,
                object.local_bounds.max.y,
                object.local_bounds.min.z,
            ),
            Point3::new(
                object.local_bounds.max.x,
    ...
    fn update_dirty_objects(&mut self) -> Result<()> {
        for &object_key in &self.dirty_objects {
            if let Some(object) = self.objects.get(object_key) {
                // PARITY_NOTE: C++ W3DTreeBuffer.cpp doesn't track dirty objects.
                // C++ uses a flat tree array with partition bucket lookup via getPartitionBucket().
                // Trees are only removed when explicitly deleted. No spatial re-insertion on move.
                // This Rust implementation's octree needs re-insertion: remove from old node,
                // find new containing node based on updated position, insert there.
                // For now, trigger full rebuild if too many dirty objects accumulate.
    ...
    fn perform_culling(&mut self) -> Result<()> {
        ...
        for object in self.objects.values_mut() {
            ...
            // PARITY_NOTE: C++ W3DTreeBuffer.cpp:269 cull() only does frustum culling
            // via camera->Cull_Sphere(m_trees[curTree].bounds). No occlusion culling exists in C++.
            // The C++ code does compute sortKeys (distance along camera look-at vector) for
            // alpha-sorting during render. Occlusion culling is a Rust-only enhancement.
    ...
    fn create_render_batches(&mut self) -> Result<()> {
        ...
        for (object_key, object) in &self.objects {
            ...
            let batch_index = if let Some(&index) = self.batch_map.get(&batch_key) {
                ...
                let new_batch =
                    RenderBatch::new(object.mesh_id, object.material_id, object.lod_level);
        ...
        self.transparent_batches
            .sort_by(|a, b| b.average_distance.partial_cmp(&a.average_distance).unwrap());

        // Sort opaque batches by state changes (material, then mesh)
        self.render_batches
            .sort_by_key(|batch| (batch.material_id, batch.mesh_id));

        // Update GPU instance buffers
        self.update_instance_buffers()?;

        self.metrics.batches_created =
            self.render_batches.len() as u32 + self.transparent_batches.len() as u32;
            ...
            if !batch.instance_data.is_empty() {
                if batch.instance_buffer.is_none() {
                    ...
                    let buffer = device.create_buffer(&BufferDescriptor {
                        ...
                        size: (batch.instance_data.len() * std::mem::size_of::<Matrix4<f32>>())
                            as u64,
                        usage: BufferUsages::VERTEX | BufferUsages::COPY_DST,
                        mapped_at_creation: false,
                    });
