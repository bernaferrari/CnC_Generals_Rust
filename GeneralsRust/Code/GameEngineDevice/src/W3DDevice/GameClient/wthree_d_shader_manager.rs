...
pub trait ScreenFilter: Send + Sync {
    ...
    /// Pre-render setup
    fn pre_render(&mut self, skip_render: &mut bool, scene_pass_mode: &mut CustomScenePassMode) -> bool;
    /// Post-render processing
    fn post_render(&mut self, mode: FilterMode, scroll_delta: &mut Vector2<f32>, do_extra_render: &mut bool) -> bool;
...
impl ScreenFilter for DefaultScreenFilter {
    ...
    fn pre_render(&mut self, _skip_render: &mut bool, _scene_pass_mode: &mut CustomScenePassMode) -> bool {
        self.initialized
    }
    
    fn post_render(&mut self, _mode: FilterMode, _scroll_delta: &mut Vector2<f32>, _do_extra_render: &mut bool) -> bool {
        self.initialized
    }
...
impl ScreenFilter for BlackWhiteFilter {
    ...
    fn pre_render(&mut self, skip_render: &mut bool, _scene_pass_mode: &mut CustomScenePassMode) -> bool {
        *skip_render = false;
        true
    }
    
    fn post_render(&mut self, mode: FilterMode, _scroll_delta: &mut Vector2<f32>, _do_extra_render: &mut bool) -> bool {
...
impl ScreenFilter for MotionBlurFilter {
    ...
    fn post_render(&mut self, _mode: FilterMode, scroll_delta: &mut Vector2<f32>, do_extra_render: &mut bool) -> bool {
...
impl ScreenFilter for CrossFadeFilter {
    ...
    fn post_render(&mut self, _mode: FilterMode, _scroll_delta: &mut Vector2<f32>, do_extra_render: &mut bool) -> bool {
...
impl W3DShaderManager {
    ...
    fn register_default_shaders(&mut self) {
        self.shaders.insert(ShaderType::TerrainBase, ShaderDescription {
        ...
        self.shaders.insert(ShaderType::TerrainBaseNoise1, ShaderDescription {
        ...
        self.shaders.insert(ShaderType::ShroudTexture, ShaderDescription {
        ...
        self.shaders.insert(ShaderType::CloudTexture, ShaderDescription {
    ...
    pub fn init(&mut self) -> Result<(), ShaderManagerError> {
        ...
        // Initialize filters
        let default_filter = Arc::new(RwLock::new(Box::new(DefaultScreenFilter::new()) as Box<dyn ScreenFilter>));
        default_filter.write().unwrap().init()?;
        self.filters.insert(FilterType::ViewDefault, default_filter);
        
        let bw_filter = Arc::new(RwLock::new(Box::new(BlackWhiteFilter::new()) as Box<dyn ScreenFilter>));
        bw_filter.write().unwrap().init()?;
        self.filters.insert(FilterType::ViewBwFilter, bw_filter);
        
        let motion_blur = Arc::new(RwLock::new(Box::new(MotionBlurFilter::new()) as Box<dyn ScreenFilter>));
        motion_blur.write().unwrap().init()?;
        self.filters.insert(FilterType::ViewMotionBlur, motion_blur);
        
        let cross_fade = Arc::new(RwLock::new(Box::new(CrossFadeFilter::new()) as Box<dyn ScreenFilter>));
...
mod tests {
    ...
    fn test_bw_filter() {
        ...
        filter.post_render(FilterMode::ViewBwBlackAndWhite, &mut scroll_delta, &mut do_extra_render);
    }
