...
`writeback_construction_to_host` is construction/sell/rebuild only; host `construction_percent` always accumulates (construction authority no longer freezes percent — that stalled map-world barracks completion). Golden map combat materializes `host_damage_log` into host HP/destroy when no shadow writeback session runs.; combat status, AI state, contain, supplies, veterancy, and special-power use dedicated session writebacks.
