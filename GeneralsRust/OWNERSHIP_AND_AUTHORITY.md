...
- Deferred-destroy bridge: GameWorld `Destroy` marks same-frame (`destroyed` + `destroyed_at_frame`, still findable by id) and `process_destroy_list` removes at host process; shadow probe adds pose/attack/move/weapon/contain/destroy-visibility (warning-only). Opt out `GENERALS_GAMEWORLD_DEFERRED_DESTROY=0`.
- Phase 2 weapon slots: GameWorld owns per-slot clip/ammo/reload/last_fire/barrel/lock for primary/secondary/tertiary/mine-clear (`SetWeaponSlot`; tertiary never aliases primary). Host is last-writer reader via writeback. Entity keeps primary residual only (entities/mod.rs not widened). Fail-closed if host has no slot source. Opt out `GENERALS_GAMEWORLD_WEAPON_AUTHORITY=0`. Probe `weapon_match` covers all slots (warning-only).
- Phase 3 movement: `GameWorld::step_movement` is the pose integrator while shadow is live (`GENERALS_GAMEWORLD_MOVEMENT_AUTHORITY` default on; host `update_movement` skips when `gameworld_movement_authority_live()`). Host writes pose back from GW. Probe `pose_match` epsilon 1e-3.
- Phase 4 factory/construction: GameWorld sole-ticks queue `construction_frames` once per logic update (`ProductionUpdate.cpp:687`) and door phase (`updateDoors` at `:513`) under existing `GENERALS_GAMEWORLD_PRODUCTION_AUTHORITY`; construction percent under existing `GENERALS_GAMEWORLD_CONSTRUCTION_AUTHORITY`. No new env names. Host writeback is last-writer reader. Probe `production_match` is warning-only (head frames + door phase). Host still allocates ObjectId / completes spawns.
