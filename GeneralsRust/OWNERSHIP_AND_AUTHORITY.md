...
| Concern | Production default | Switch |
|---------|-------------------|--------|
| Dual `gamelogic` crate tick | **removed** (`AuthorityOnly` only) | — |
| Shadow session | **on** | opt out `GENERALS_GAMEWORLD_SHADOW=0` |
| Entity live modules | **off** (opt-in attach at spawn; `on_delete` before deferred remove) | opt in `GENERALS_GAMEWORLD_ENTITY_MODULES=1` |
| HP last-writer (damage auth) | **off** — host `GameLogic` is the sole HP writer (C++ one store) | opt in `GameLogic::set_damage_authority(true)` |
| Movement integrate (path follow) | **off** — host `update_movement` always integrates | opt in `GameLogic::set_movement_authority(true)` |
| Construction percent | **off** — host accumulates percent (rebuild holes set host percent immediately) | opt in `GameLogic::set_construction_authority(true)` |
| Cash last-writer (economy auth) | **off** — host mutates supplies immediately | opt in `GameLogic::set_economy_authority(true)` |
| Attack target channel (shadow↔host) | **off** (`ai_attack` context field; writeback hard-returns 0 by default) | opt in `GameLogic::set_ai_attack_authority(true)` |
| Move destination channel (shadow↔host) | **off** (`movement` context field) | opt in `GameLogic::set_movement_authority(true)` |
...
| Full `GameClient::update()` | Main still prefers presentation shell; host-safe if empty registry | shell polls input+audio |

Authority channels are per-`GameLogic` context fields (`gameworld_authority.rs`, hq-e84zk):
every channel — damage / economy / movement / ai_attack / projectile / ai_decision /
fire_spawn / construction / special_power / production / weapon — defaults **off**
(host sole writer, C++ parity). The retired `GENERALS_GAMEWORLD_*_AUTHORITY` env
flags no longer exist. The wave-numbered phase notes below were written against
older gates and may lag the code; the code (and this table) is the truth.

- Target end state: `gamelogic::GameWorld` sole host; Main = composition root only.
- Deferred-destroy bridge: GameWorld `Destroy` marks same-frame (`destroyed` + `destroyed_at_frame`, still findable by id) and `process_destroy_list` removes at host process; shadow probe adds pose/attack/move/weapon/contain/destroy-visibility (warning-only). Opt in `GENERALS_GAMEWORLD_DEFERRED_DESTROY=1` (default off).
- Phase 2 weapon slots: GameWorld owns per-slot clip/ammo/reload/last_fire/barrel/lock for primary/secondary/tertiary/mine-clear (`SetWeaponSlot`; tertiary never aliases primary). Host is last-writer reader via writeback. Entity keeps primary residual only (entities/mod.rs not widened). Fail-closed if host has no slot source. Opt in `GameLogic::set_weapon_authority(true)` (default off). Probe `weapon_match` covers all slots (warning-only).
- Phase 3 movement: `GameWorld::step_movement` is the pose integrator only while movement authority is live (`GameLogic::set_movement_authority(true)`, default off; host `update_movement` skips integrate when `gameworld_movement_authority_live()`). Host writes pose back from GW. Probe `pose_match` epsilon 1e-3.
- Phase 4 factory/construction: GameWorld sole-ticks queue `construction_frames` once per logic update (`ProductionUpdate.cpp:687`) and door phase (`updateDoors` at `:513`) under `GameLogic::set_production_authority(true)`; construction percent under `GameLogic::set_construction_authority(true)`. Both default off. Host writeback is last-writer reader. Probe `production_match` is warning-only (head frames + door phase). Host still allocates ObjectId / completes spawns.
- Phase 5 contain: occupant roster lives in `world/contain_roster.rs` (Entity host-id residual unchanged; `entities/mod.rs` not edited). `ContainEnter`/`ContainExit` are mutations. `process_destroy_list` ejects occupants before remove. Probe `contain_match` includes roster equality (warning-only).
- Phase 6 commands: while shadow is coupled, `ingest_command_queue_view` writes `SetAttackTarget`/`SetMoveTarget`/`PushAiDecision` from the host `command_queue` without popping it (queue remains a view). Host `process_commands` still runs C++ side effects. A queued attack lands in GW the same tick.
- Current honesty: Main still owns mid-frame AI *execution* (fire-intent residual last-writes via SetFireIntent when GENERALS_GAMEWORLD_AI_ATTACK_AUTHORITY on (default; fire_at defers host last_fire_* until writeback); weapon fire-spawn via GENERALS_GAMEWORLD_FIRE_SPAWN_AUTHORITY (default on); fire decision still host-side (residual auto-fire now emits host_ai_decision_log AttackTarget/SetAIState under AI_DECISION_AUTHORITY for GameWorld last-write channel; host still chooses residual targets via pure host_residual_acquire query (combat: base defense/sentry/hellfire/garrison/transport/strategy-center-bombardment+mood/gattling-impact/point-defense-intercept/continue-attack/repulsor/mine-clearer/money-crate-picker/return-to-base-rearm/supply-center/click-select/patriot-assist/command-executor-garrison-airfield-supply-mine/ai-nearest-enemy/supply-source/dozer-boot/spectre-gattling/command-integration-mouse-pick/dozer-presentation/unit-control-presentation-pick/engine-boot-mouse-pick/ai-factory-pick; service: heal/repair/resume-construction/dozer-bored/battle-drone-master/harvest-supply; ground-attack-impact; pilot: find-vehicle recrew); Main presentation-first dual-scans label live get_objects paths as Boot residual only (or Fail-open live residual); named-object lookup/transfer/attack-priority sync prefer host ObjectId when engine_object bridge off (default); command_move/attack host path does not require engine_object_id (bridge residual only); residual→decision→GameWorld writeback sets host attack target under AI_ATTACK_AUTHORITY); residual auto-fire (incl. strategy-center AoE + patriot assist) queues fire-spawn channel with live damage under FIRE_SPAWN_AUTHORITY; hitscan take_damage_from keeps same-frame residual HP and records residual-hitscan pairs so shadow zeroes spawn damage (no dual-tick double-dip); ballistic projectile-owned residual HP deferred until dual-tick tests drive shadow resolve; residual auto-fire stamps weapons via consume_ammo_on_fire (clip/ammo parity with fire_at); residual auto-fire records host_attack_log for Presentation AttackTargeted/WeaponFire; outpost/crawler payload + freefall eject log move/ground under MOVEMENT_AUTHORITY; create_object/UC spawn + capture-kick + paradrop elevate log move under MOVEMENT_AUTHORITY; residual auto-fire + fire_at pre-attack log fire-intent/SetAIState under attack/decision authority; residual auto-fire damage uses take_damage_from(source) for BodyModule last_damage_source + damage log; combat target clears (private_stop, guard-lost, capture/eject, mood clear) use StopAttack decision channel; angry-mob/PDL/DoT-zone residual damage source-attributed + PDL fire-intent/attack log; explosion/detonation/sneak/overcharge residual damage source-attributed; production-cancel refund + AI starting-cash top-up use economy authority pending delta; sell/rebuild construction percent progress logs under CONSTRUCTION_AUTHORITY; heal-crate/armor/battle-plan/drone-repair absolute HP via heal channel under DAMAGE_AUTHORITY; crash/chute lethal HP + rebuild start HP honor damage authority; command_attack range-snap + RTB/takeoff pad snaps suppressed under MOVEMENT_AUTHORITY; RenderPipeline::execute packs presentation laser/projectile/order-line CPU segments; + floating-text/world-anim/particle-system CPU layouts; presentation audio dispatches direct to AudioManager (no GameLogic dual-write; no dual engine SFX; presentation shell drains client audio queue; GameClient mouse+keyboard device inject from Main OS events into shared THE_MOUSE/THE_KEYBOARD (presentation shell ticks update_input on shared handles; Main still owns commands); SELECT_HERO + combat/moving/attacking/guard/patrol/gather/stealth/veteran + harvester/transport/docked/repair/construct/idle-military + cycle damaged/busy/ready + stop-all filter-select hotkeys prefer presentation ids; snap_camera_to_local_units prefers presentation poses; runtime-host select_local_unit/attack_nearest_enemy/guard empty-pick prefer presentation mobiles/combat; sell_selected empty targets prefer presentation sellable structures; queue_upgrade/construct empty producer-builder picks prefer presentation; scatter/formation/return_to_supply/set_rally/overcharge/cancel_queue/select_similar/attackmove empty picks prefer presentation; force_attack_object enemy + attack_nearest enemy prefer presentation; construct dozer-spawn/loc CC + train unfinished-barracks discovery prefer presentation; train producer prefers force-completed + presentation barracks; cycle idle/busy workers + unfinished construction + resume construction prefer presentation; runtime_host_status_snapshot mobile/UC/sample prefer presentation; play_sound_effect UI SFX also direct when frame installed); EVA counters freeze on PresentationFrame (HUD sync prefers snapshot); suicide/consume destroy residuals log lethal HP under DAMAGE_AUTHORITY; parachute freefall residual logs ground height + landing move dest under MOVEMENT_AUTHORITY (host pose stays freefall residual); map-ground/support-container/building-rubble eject logs ground-height + move dest under MOVEMENT_AUTHORITY; AI decision *apply* via GENERALS_GAMEWORLD_AI_DECISION_AUTHORITY (default on; includes mood auto-acquire, support-state guard engage, faction AI launch_attack, and AI residual stop_attack; apply_ai_command is log-only for Attack/SetState when authority on; continue-attack-after-kill logs next target; assign_unit_attack_path logs Attack/Attacking while path stays host; path_approach_with_state logs SetAIState; assign_unit_path/move_object_with_pathfinding log Moving/state while path stays host; private_idle + support Idle clears log StopAttack/Idle; RTB Docked, min-range backup, waypoints, attack-enter, crate pickup, selling-container Idle gate SetAIState; dozer/construction/rebuild/destroy-list AI state gated (non-combat targets stay host); capture/eject/hijacker Docked + strategy-center hold/clear + combat approach/ground state gated; bunker/kill-garrisoned/freefall/parachute-land/hive Idle; angry-mobs Attacking; mine clear Moving/Idle; sell/cancel-dozer Idle; resume Constructing; payload Docked; player command_attack/stop state gated; dozer/construction/rebuild/destroy-list AI state gated (non-combat targets stay host); troop-crawler assault + missile-defender laser engage log AttackTarget; attack_state_enter/privateAttackObject log engagement; transfer_attack logs retarget; update_combat aim/pitch/pre-attack skips host target mutate when authority on; private_face_object logs AttackTarget; residual defense/drone/patriot/nested-SM fire gates engagement; AttackStateMachine tick + strategy mood + camo-netting order gate engagement); projectile flight residual mirrors via SetProjectileFlight; AICommand order buffer via PushAiDecision; projectile flight integrate via GENERALS_GAMEWORLD_PROJECTILE_AUTHORITY); damage/heal/XP/construction-HP authority defers host HP/XP mutate to GameWorld writeback; economy authority uses pending_supply_delta (spend/add/credit/income/bounty/refund) then absolute writeback; path integrate defaults to GameWorld movement authority (GENERALS_GAMEWORLD_MOVEMENT_AUTHORITY); construction percent defaults to GameWorld construction authority (GENERALS_GAMEWORLD_CONSTRUCTION_AUTHORITY); special-power timers log each tick_timers for SetSpecialPower mirror; production queue progress logs each factory tick for SetProductionQueue; weapon last_fire_time residual on SetWeaponStats; factory exit_delay_remaining on SetExitDelay; body_damage_state on SetBodyDamage; weapon clip_size/clip_reload_time on SetWeaponStats; front/back_crushed on SetCrushVision; waiting_for_path on SetMovement; death_type on SetDeathType; object radar_extend on SetRadarExtend; locomotor/path flags on SetMovement; shock_stun residual on SetShockStun; blocked-path residual on SetMovement; sole-healing benefactor on SetSoleHealing; AI mood/idle on SetAiMood; guard_radius on SetGuard; production door on SetProductionDoor; rebuild hole identity; physics/motive on SetPhysicsMotive; bounce/land on SetBounceLand; extended turret residual on SetTurret; extended physics friction/overlap; stealth delay/camo pulse on SetStealthDelay; combat pre-attack/AttackStateMachine on SetCombatAttack; locomotor/path-approach on SetLocomotor; AI request/retaliate/crate on SetAiRequest; hijacker/hive-respawn on SetHijacker; weapon leech-range on SetWeaponStats; physics_turning on SetLocomotor; GameWorld is last-writer for HP/max_health/experience/weapon_bonus/frenzy_until/battle_plan_sight/active_weapon_slot/entity_power/turret/target_location/detector/continuous_fire/guard/ai_attitude/weapon_set/overcharge/contain_capacity/hive_slaves/stealth_flags/overlord_addon/command_set/disguise/vision_camo/weapon_stats/movement/selection_radius/model_condition/demo_mine_cheer/crush_vision/combat_status/building_type/identity/ground_height/model_mesh/fow/kind_of_bits/cash/power/radar/shared-SP-cooldowns/alive/bounty/rank/pose/targets/production/construction/completed_upgrades/status (stealth/disable/emp/jam/mask/disguise/move/attack/select) via writeback + SetCombatStatus mutations (host_status_log selection/attack/move/fire/aim/stealth/detect/emp/jam/hack/unmanned/paralyze/subdue/mask/disguise/no_collisions/private_captured/disguise_transition/faerie/booby/parachute/force_attack/using_ability/deployed/construction/veterancy/production_queue/owner/construction_pct/special_power+cooldown/active_weapon_slot/stored_supplies/ai_state/contain/occupant_count/player_radar/player_progress/player_meta/completed_upgrades/max_health/experience/weapon_bonus) + presentation overlay.
- Draw/render path is presentation-only (no live `Option<&GameLogic>` dual-read on execute/collect/selection/terrain).

`writeback_construction_to_host` is construction/sell/rebuild only; host `construction_percent` always accumulates (construction authority no longer freezes percent — that stalled map-world barracks completion and multi-frame sell deconstruction). Capture residual applies host Idle immediately while still logging AI_DECISION. Breadth/sell refunds assert via `effective_supplies` under economy authority. Golden map combat materializes `host_damage_log` into host HP/destroy when no shadow writeback session runs.; combat status, AI state, contain, supplies, veterancy, and special-power use dedicated session writebacks.
...
- Presentation build + client/render orchestration:
  ...
  - Loading client: `host_tick_loading_client_residuals` (Wave 604)
  - UI SFX: `host_play_sound_effect` (Wave 604)
  - Menu client: `host_tick_menu_client_residuals` (shell tick/commands/camera/menu shell; Wave 605)
  - OS inject: `host_inject_game_client_*` (Wave 606)
  ...
  - Eager AI-state/fire-intent: post-logic drain `host_ai_state_log`/`host_fire_intent_log` → GW (Wave 687; session handoff)
  - Eager owner/movement: post-logic drain `host_owner_log`/`host_movement_log` → GW (Wave 688; session handoff)
  - Eager status/veterancy: post-logic drain `host_status_log`/`host_veterancy_log` → GW (Wave 689; session handoff)
  - Eager weapon-bonus/slot: post-logic drain `host_weapon_bonus_log`/`host_weapon_slot_log` → GW (Wave 690; session handoff)
  - Eager weapon-set/entity-power: post-logic drain `host_weapon_set_log`/`host_entity_power_log` → GW (Wave 691; session handoff)
  ...
  - Eager body-damage/death-type/physics-motive: post-logic drain residual logs → GW (Wave 702; session handoff)
  - Eager locomotor/bounce-land: post-logic drain residual logs → GW (Wave 703; session handoff)
  - Eager AI-mood/AI-request/shock-stun: post-logic drain residual logs → GW (Wave 704; session handoff)
  - Eager stealth-delay/sole-healing/radar-extend: post-logic drain residual logs → GW (Wave 705; session handoff)
  - Eager hijacker/rebuild-producer/stored-supplies: post-logic drain residual logs → GW (Wave 706; session handoff)
  ...
  - Same-frame construction complete after GW ready writeback under sole-tick (Wave 715)
  ...
  - formation free buddy-infantry spawn is opt-in only (Wave 720; smoke sets spawn_buddy=1)
  ...
  - damage authority: Spectre prior-clear does not stomp host HP mid-frame (Wave 750; damage log + flags)
  - damage authority: booby-trap special destroy does not stomp host HP mid-frame (Wave 751; damage log + flags)
  ...
  - writeback skips pending remaining host logs under coupled tick (Wave 758; ~36 writebacks incl. health/contain/owner)
  ...
  - Sell-finish ready log: GW writeback → `host_sell_ready_log` → host finish gate (Wave 619)
  ...
  - Upgrade-complete ready log: GW writeback → `host_upgrade_ready_log` → host apply_host_upgrade_complete (Wave 624)
  - Radar-extend ready log: GW writeback → `host_radar_extend_ready_log` → host model/counter apply (Wave 625)
  - Construction-complete-clear ready log: GW writeback → `host_construction_complete_clear_ready_log` → host bit clear (Wave 626)
  ...
  - Economy ready log: GW writeback → `host_economy_ready_log` → host presentation bookkeeping (Wave 631)
  - Death-type ready log: GW writeback → `host_death_type_ready_log` → host destroy/pilot bookkeeping (Wave 632)
  - Model-condition ready log: GW writeback → `host_model_condition_ready_log` → host presentation bookkeeping (Wave 633)
  - Combat-status ready log: GW writeback → `host_combat_status_ready_log` → host status presentation bookkeeping (Wave 634)
  - Weapon-stats ready log: GW writeback → `host_weapon_stats_ready_log` → host weapon presentation bookkeeping (Wave 635)
  ...
  - Fire-intent ready log: GW writeback → `host_fire_intent_ready_log` → host fire presentation bookkeeping (Wave 640)
  - Stored-supplies ready log: GW writeback → `host_stored_supplies_ready_log` → host gatherer presentation (Wave 641)
  ...
  - Selection Radius ready log: GW writeback → `host_selection_radius_ready_log` → host selection radius presentation (Wave 655)
  - Ground Height ready log: GW writeback → `host_ground_height_ready_log` → host ground height presentation (Wave 656)
  - Weapon Slot ready log: GW writeback → `host_weapon_slot_ready_log` → host weapon slot presentation (Wave 657)
  - Weapon Bonus ready log: GW writeback → `host_weapon_bonus_ready_log` → host weapon bonus presentation (Wave 658)
  - Ai Attitude ready log: GW writeback → `host_ai_attitude_ready_log` → host AI attitude presentation (Wave 659)
  - Identity ready log: GW writeback → `host_identity_ready_log` → host identity presentation (Wave 660)
  - Repulsor ready log: GW writeback → `host_repulsor_ready_log` → host repulsor presentation (Wave 661)
  - Shock Stun ready log: GW writeback → `host_shock_stun_ready_log` → host shock stun presentation (Wave 662)
  - Sole Healing ready log: GW writeback → `host_sole_healing_ready_log` → host sole healing presentation (Wave 663)
  - Crush Vision ready log: GW writeback → `host_crush_vision_ready_log` → host crush vision presentation (Wave 664)
  - Demo Mine Cheer ready log: GW writeback → `host_demo_mine_cheer_ready_log` → host demo mine cheer presentation (Wave 665)
  - Overlord ready log: GW writeback → `host_overlord_ready_log` → host overlord presentation (Wave 666)
  - Hive ready log: GW writeback → `host_hive_ready_log` → host hive presentation (Wave 667)
  - Overcharge ready log: GW writeback → `host_overcharge_ready_log` → host overcharge presentation (Wave 668)
  - Guard ready log: GW writeback → `host_guard_ready_log` → host guard presentation (Wave 669)
  - Continuous Fire ready log: GW writeback → `host_continuous_fire_ready_log` → host continuous fire presentation (Wave 670)
  - Detector ready log: GW writeback → `host_detector_ready_log` → host detector presentation (Wave 671)
  - Target Location ready log: GW writeback → `host_target_location_ready_log` → host target location presentation (Wave 672)
  - Turret ready log: GW writeback → `host_turret_ready_log` → host turret presentation (Wave 673)
  - Entity Power ready log: GW writeback → `host_entity_power_ready_log` → host entity power presentation (Wave 674)
...
- Audio: presentation event dispatch via finalize helper; UI SFX via `play_sound_effect`; background music sink ownership on Main
