...
Gates call `ensure_gate_damage_authority()` so damage authority defaults on (set `GENERALS_GAMEWORLD_DAMAGE_AUTHORITY=0` to opt out). `GENERALS_GAMEWORLD_ECONOMY_AUTHORITY=1` (gates default-on): `host_economy_log (includes power from `update_player_resources`)` from Player spend/add/bounty/refund → SetSupplies/SetPower mutations → host writeback. `GENERALS_GAMEWORLD_DAMAGE_AUTHORITY=1` (implies shadow session): end-of-tick
