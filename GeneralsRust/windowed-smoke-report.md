...
## ConstructCloser — construct pad gates closed per C++ (construct_ok live ×4), walk leg blocked by live path-queue drain defect + capture-walk regression handed off (2026-09-04 14:20-02:45)

### Fixes LANDED (all C++-cited; scope: construct gate parity)
1. **Build-gate CLEAR_PATH now uses the C++ ForUI variant** (`ui_production.rs builder_has_clear_path_to` → `quick_path_available_residual`): BuildAssistant.cpp:987 `ai->isQuickPathAvailable` → AIUpdate.cpp:2080-2091 → `Pathfinder::clientSafeQuickDoesPathExistForUI` (AIPathfind.cpp:8055-8103) — TERRAIN-zone compare that explicitly does not take structures into account (:8079-8081) and fail-opens on UNINITIALIZED zones (:8073-8078). The previous structure-aware `clientSafeQuickDoesPathExist` (AIPathfind.cpp:7997) is the AIPlayer.cpp:596 teleport-check site, not the build gate. This removed the 367-candidate LBC_NO_CLEAR_PATH ring (FinalCloser census) — scan now finds `pad ok` candidates on the first ring.
2. **moveObjectsForConstruction candidates are shape-exact** (`ui_production.rs move_objects_for_construction`): C++ iterates `iteratePotentialCollisions(pos, gi, angle)` whose filter IS PartitionFilterWouldCollide (PartitionManager.cpp:3619-3622) — the same wouldCollide test isLocationClearOfObjects uses; query geometry = authored BOX else BOX(major,major) (BuildAssistant.cpp:1397-1401); scoot radius sqrt(major²+minor²)*1.4 (:1409-1410). The old radius-sum circle flagged authored-shape neutral props that legally miss the pad → `exec abort move_objects` (live-attributed by the [CP] exec probe).
3. **Dock snap before the walk** (`construct.rs execute_build` → `find_good_build_or_repair_position`): C++ newTask → findGoodBuildOrRepairPosition half-radius seed + findPositionAround snap (DozerAIUpdate.cpp:1855-1894). The raw seed sits on the building's own pathfind-blocked footprint.
4. **Walk order = Moving, build state only at dock** (`construct.rs` walk): C++ DOZER_DO_BUILD_AT_DOCK (DozerAIUpdate.cpp:499-507) — dozer walks in its move machine, flips to AI_CONSTRUCT on arrival (mirrors the line-build walk pattern). Constructing-before-walk froze the movement follower.
5. **Pathfind queue drained every logic frame inside `update_simulation`** (`world_tick/step.rs`): C++ AI::update → processPathfindQueue (AI.cpp:332-339). The drain previously lived ONLY in `world_runtime::update()`, which the live host never calls (live ticks via `tick_logic_frame` → `update_simulation`) — every deferred path on a loaded map stayed `waiting_for_path=true` forever: units never walked (dozer, march leg, rhd2 move markers, EconomyE2E walks — all one defect).
6. **Throttled dozer approach re-issue** (`production.rs reissue_dozer_approaches`, called from update_simulation tail): C++ PICK_ACTION_POS re-issues the approach until arrival (DozerAIUpdate.cpp:318-348); keyed on the durable `dozer_task_build_target` (m_task[DOZER_TASK_BUILD]) so ephemeral `target` clears cannot orphan a scaffold.

### Drive evidence (Defcon6, USA vs GLA, real map)
- `construct_ok:AmericaBarracks@2987.99,2121.34` reached **4×** across fix-verification drives (pad found by the auto scan on ring 1 at (2988,2121); `[CP] exec success building=468 dozer=465` — building created under construction).
- Final census on the fixed gates: scan `{ok=1}` on first candidate (was `{OK:0}` over 1352 candidates pre-fix).
- Earlier blockers confirmed fixed: pocket IMMOBILE objects no longer over-block (trees/bushes SHRUBBERY parity + authored geometry landed by FinalCloser remain correct); CLEAR_PATH ring eliminated by fix 1; SHROUD rejects are C++-correct human-gate behavior at unrevealed outskirts.

### Walk leg — blocked by ONE remaining live defect, fully diagnosed, handed off
- Symptom: after `exec success`, the dozer sits at its spawn pose (vel 0, path installed len 4, `waiting_for_path` true forever live).
- Root cause chain: (a) live never drained the path queue (fix 5 — now in tree); (b) headless repro (`construct_probe_tmp.rs`, removed) additionally showed `dozer_task_build_target`/`target` wiped by frame 1 in the synthetic harness — headless cannot validate the walk leg; (c) live, even with a correctly installed path and a fresh re-issue, pose does not integrate — movement-follower vs coupled-GameWorld-shadow authority needs its own lane (evidence: `[CP] tick` probes show dist=109 frozen across minutes; control assigns from outside the tick DO walk — control-A/B walked 114→0 and 160-east in the same session).
- Therefore `under_construction 1→0 → Ranger → march → attack_ok` did NOT latch this window; the march leg and rhd2's move markers share this defect. Recommend a dedicated movement-authority lane (FixMove + shadow owner) with my evidence: /tmp/wsmoke/g3.log, $(cat /tmp/wsmoke/g3_dir)/child_stderr.txt (`[CP] tick`/`reissue` lines), and the removed repro test (recoverable from this report's git-less tree description: Defcon6 load + spawn_skirmish_starting_units + exec-order replay).

### Capture-walk regression surfaced (NOT mine — handed to Main → SpecialPowerFix)
`capture_and_containment::capture_building_walk_into_range_transfers_ownership_after_upgrade` (33-test combat family) broke during the afternoon wave. A/B evidence: my step.rs drain relocation reverted → still fails; FixMove reverted all 4 movement.rs changes → still fails; my pathfinding ignore-threading is None-neutral. Test drives the RangerCaptureBuilding special-power walk→channel (SpecialPowerFix's lane). Passed 33/33 at 14:50 on my build.

### Cleanup (done)
- ALL [CP]/[CP-TMP] probes removed (gameplay_select.rs scan census + helper, ui_commands.rs confirm-reject, construct.rs exec probes + helper, production.rs ucon/tick/reissue diagnostics); `construct_probe_tmp.rs` deleted + unwired from world_tests/mod.rs. grep GENERALS_CONSTRUCT_PROBE = 0 in Code/.
- Probes kept by other lanes (untouched): GENERALS_SLUI_PROBE, GENERALS_RIGHTHUD_PROBE (VerifyBatch staged).

### Guards (final tree, probe-free)
- `structure_placement` 14/14; `capture_and_containment` 32/33 (1 walk-dependent failure = handed-off regression above, was 33/33 before the sibling wave); `zones` 10/10; `dozer_construct_places_barracks_under_construction` green; `cargo check -p generals_main --lib` + `--tests` 0 errors.
- No git writes; no formatters/linters; drives serial with caffeinate; sibling lanes coordinated via hub (VerifyBatch, FixMove, EvaVoiceFix, SpecialPowerFix, DeathFxFix, EconomyE2E, FixCtrlBar hud.rs E0063 fix acknowledged).
